...
function Table({ className, ...props }: React.ComponentProps<"table">) {
  return (
    <div className="relative w-full overflow-x-auto" data-slot="table-container">
      <table className={cn("w-max min-w-full caption-bottom text-[13px]", className)} data-slot="table" {...props} />
    </div>
  );
}
...
function TableRow({ className, ...props }: React.ComponentProps<"tr">) {
  return (
    <tr
      className={cn(
        "border-b border-border transition-colors hover:bg-accent has-aria-expanded:bg-selected data-[state=selected]:bg-selected",
        className,
      )}
      data-slot="table-row"
      {...props}
