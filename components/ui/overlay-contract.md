...
- Mark every interactive assistant surface (the panel and its tour popover) with `data-agent-surface`. While the panel or its guidance is on screen, AppModal renders non-modal, and AppModal and ResponsiveOverlay ignore presses and focus inside `[data-agent-surface]`, so the user can answer the assistant or step its tour while the dialog it points into stays open. A non-modal AppModal also ignores focus leaving it, so a confirmation, search, or assistant prompt opened over it never closes it; only an outside press or Escape does. An AppModal launched from the assistant passes `layerClassName={OVERLAY_TOPMOST_LAYER_CLASS}` and stays modal.
