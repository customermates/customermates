...
import { IllustrationMotion } from "./illustration-motion";

const BODY = {
  "data-illustration-part": "body",
  fill: ILLUSTRATION_INK,
  opacity: ILLUSTRATION_OPACITY.body,
};

const DETAIL = {
  "data-illustration-part": "detail",
  fill: ILLUSTRATION_INK,
  opacity: ILLUSTRATION_OPACITY.detail,
};

const EDGE = {
  "data-illustration-part": "edge",
  fill: "none",
...
const ACCENT = { "data-illustration-part": "accent" };

const MARK = { "data-illustration-part": "mark" };

function Edge(props: ComponentProps<"path">) {
  return <path {...props} pathLength={1} />;
}

function Illustration({ children, className, label }: IllustrationProps & { children: ReactNode }) {
  return (
    <IllustrationMotion>
      <svg {...illustrationSvgProps({ className, label })}>{children}</svg>
    </IllustrationMotion>
  );
}
...
export function ChannelsConverge(props: IllustrationProps) {
  ...
  return (
    <Illustration {...props}>
      <g {...EDGE}>
        {rows.map((y) => (
          <Edge key={y} d={`M188 ${y + 20}C260 ${y + 20} 280 192 352 192`} />
        ))}
      </g>
...
export function FlowSchematic(props: IllustrationProps) {
  ...
  return (
    <Illustration {...props}>
      <g {...EDGE}>
        <Edge d="M190 108C218 108 218 180 240 180" />

        <Edge d="M190 252C218 252 218 180 240 180" />

        <Edge d="M395 180h50" />

        <Edge d="M232 174l6 6-6 6" strokeLinejoin="round" />

        <Edge d="M437 174l6 6-6 6" strokeLinejoin="round" />
      </g>
