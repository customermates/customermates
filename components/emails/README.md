...
The automated preview manifest inventories every production send and renders each recipient-facing behavior in `en`, `de`, `fr`, `it`, and `es`; operator notifications intentionally stay in English. Every address, token, company, message, and URL in the fixtures is synthetic.
