...
function stubSuccessfulSend() {
  vi.stubGlobal(
    ...
    vi.fn().mockResolvedValue(
      new Response(JSON.stringify({ object: "EmailSent", id: "email-1", message_id: "message-1" }), {
        status: 200,
...
describe("HTML-only email body fallbacks", () => {
  it.each(['<img src="https://example.com/logo.png" alt="">', "<span></span>"])(
    ...
    (html) => {
      const bodies = composeEmailBodies(html, null, null, "html");
  ...
  it("passes the fallback through the MessagingService boundary as plain_text", async () => {
    stubSuccessfulSend();
    const bodies = composeEmailBodies('<img src="https://example.com/logo.png" alt="">', null, null, "html");
    ...
    expect(result).toEqual({ ok: true, data: { id: "email-1", messageId: "message-1" } });
