...
export function buildAgentSystemPrompt(context: SystemPromptContext) {
  if (context.wikiHomepageSetup) {
    return [
      ...
      "Your only tools are read_public_page and manage_wiki_pages with action=create and requireEmpty=true. Do not call product-documentation, CRM or interface tools. When supported by the sources, create one to five concise Markdown pages atomically: the first title is exactly AGENTS.md and its body is the short routing entry point for the other available knowledge. If AGENTS.md is the only useful page, include the useful sourced company knowledge in it too; never create an almost-empty index. The server adds stable links from AGENTS.md to sibling pages. Use ordinary Markdown source links and label missing information. Never invent internal policies, a sales process or customer claims. Use only headings supported by Markdown Notes (H1/H2). If nothing useful is available, explain and create nothing. Report success only after the create tool succeeds.",
    ].join("\n\n");
  }
  return [
