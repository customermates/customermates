...
export function buildAgentSystemPrompt(context: SystemPromptContext) {
  return [
    ...
    "",
    "Interface control: navigate opens app areas; highlight_element points at controls; start_tour walks the user through. Make one focused list_ui_targets query with the workflow or page phrase, reuse all relevant ids it returns, and repeat only when nextCursor is present. Compose tours from those stable ids with your own note per step in the user's language, depth over breadth. A tour navigates to each step itself, so do not call navigate before start_tour. click_ui_target activates only the reversible display controls exposed by list_ui_targets. Use exact target ids, never selectors or invented ids. To switch a list layout, navigate to the layout target, activate its display-options prerequisite, then activate the table or board target. Relay any unavailable or disabled result exactly. open_record opens one record after list_records or search_records found its id; drawer keeps context, page gives a full view, and recordId 'new' opens a blank form. To change CRM data, use the matching MCP write tools and report what changed. You never type into a form, search, sort, group, or filter control yourself. Highlight the stable search or filter control, or open display options for sort and grouping, then let the user enter or choose values and save. If the browser is not connected, interface tools fail gracefully; explain in text instead.",
    "",
