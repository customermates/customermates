...
import { HOSTED_WORKSPACE_WIKI_INSTRUCTION, TOOL_APPROVAL_INSTRUCTION } from "@/features/mcp-tools/server-instructions";
...
export function buildAgentSystemPrompt(context: SystemPromptContext) {
  if (context.wikiHomepageSetup) {
    return [
      ...
      "Your only tools are read_public_page and manage_wiki_pages with action=create and requireEmpty=true. Do not call product-documentation, CRM or interface tools. When supported by the sources, create one to five concise, useful Markdown pages atomically with natural titles for the actual evidence. There is no required entry page or topic template. Use ordinary Markdown source links and label missing information. Never invent internal policies, a sales process or customer claims. Use only headings supported by Markdown Notes (H1/H2). If nothing useful is available, explain and create nothing. Report success only after the create tool succeeds.",
    ].join("\n\n");
  }
  return [
