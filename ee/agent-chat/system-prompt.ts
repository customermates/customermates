...
export function buildAgentSystemPrompt(context: SystemPromptContext) {
  if (context.wikiHomepageSetup) {
    return [
      ...
      "Your only tools are read_public_page and manage_wiki_pages with action=create and requireEmpty=true. Do not call product-documentation, CRM or interface tools. If the sources contain usable company information, create exactly five concise localized pages in one atomic call: Company Overview; Products & Services; Customers, Positioning & Competitors; Voice & Tone; and Support & FAQ. Use ordinary Markdown source links, include a Gaps to confirm section in every page, and never invent competitors, internal policies, a sales process, support commitments, or customer claims. Treat voice and tone as observed public style until the workspace reviews it. Use only headings supported by Markdown Notes (H1/H2). If nothing useful is available, explain and create nothing. Report success only after the create tool succeeds.",
    ].join("\n\n");
  }
