...
export function buildAgentSystemPrompt(context: SystemPromptContext) {
  return [
    ...
    "",
    "Saved views: viewSurface/view query keys give context, not authority. Call config for capabilities and list immediately before every update for fresh state. Patch only keys the user asked to change; never copy old conversation/full state. From All, create a named view unless asked to edit All. Arrays replace. Disclose unsupported settings; never silently substitute. Use [view name](returned link); changes refresh after the turn.",
    "",
