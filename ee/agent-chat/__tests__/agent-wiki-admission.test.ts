...
vi.mock("@/ee/agent-chat/agent-tools", () => ({ getAgentAiToolDefinitions: definitions }));
...
function fixture() {
  const catalog = { invoke: vi.fn().mockResolvedValue({ ok: true, data: catalogData() }) };
  const repo = {
    ...
    findConversation: vi.fn().mockResolvedValue({ id: CONVERSATION_ID, origin: "routine", creditCeiling: 500 }),
    ...
    admitAgentTurnOrThrow: vi.fn().mockImplementation((args) =>
      Promise.resolve({
        ...
        recentMessages: [{ id: args.turn.userMessageId, role: "user", parts: [{ type: "text", text: args.title }] }],
      }),
    ),
  };
  const usage = {
    prepareTurn: vi
      ...
      .mockImplementation((_userId, _now, args: { model: AgentModelEntry; requiredContextBytes: number }) =>
        Promise.resolve({
          reservation: {
            budget: resolveAgentTurnBudget({ ...args, availableCredits: 500 }),
            reservedCredits: 100,
  ...
  const background = { dispatchTracked: vi.fn().mockResolvedValue("wrun_wiki_test") };
...
describe("Workspace Wiki admission bootstrap", () => {
  ...
  it.each(["chat", "routine"] as const)("loads and measures the current catalog for a %s turn", async (surface) => {
    ...
    const result = await (surface === "routine"
      ? runWithTenant(mockUser, () => state.interactor.invokeRoutine({ ...input, conversationId: CONVERSATION_ID }))
    ...
    expect(execution.messages.slice(0, 2)).toEqual(agentWikiContextMessages(payload.wikiCatalog));
  ...
  it("continues without any catalog data when Wiki Read is denied", async () => {
    ...
    const result = await state.interactor.invoke({ clientRequestId: CLIENT_REQUEST_ID, text: "Hello", retry: false });
    expect(result).toMatchObject({ ok: true, data: { disposition: "run" } });
  ...
  it("stops on a catalog validation failure before credits or workflow dispatch", async () => {
    const state = fixture();
    const failure = { ok: false, error: new z.ZodError([{ code: "custom", path: [], message: "Invalid catalog" }]) };
    state.catalog.invoke.mockResolvedValue(failure);
    expect(await state.interactor.invoke({ clientRequestId: CLIENT_REQUEST_ID, text: "Hello", retry: false })).toBe(
      failure,
    );
    expect(state.usage.prepareTurn).not.toHaveBeenCalled();
    expect(state.background.dispatchTracked).not.toHaveBeenCalled();
  ...
  it("reserves and dispatches the same 8192-token setup model without loading the private catalog", async () => {
    ...
    const admission = state.usage.prepareTurn.mock.calls[0][2];
    const payload = state.payload();
    expect(admission.model).toEqual({ ...resolveAgentModel(), maxOutputTokens: 8_192 });
