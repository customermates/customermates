...
const primaryUser = {
  ...
  avatarUrl: "https://demo.example/demo/avatars/photos/max-bergmann.png",
};
  ...
  return {
    ...
    users: [
      ...
      },
      {
        ...
        avatarUrl: "https://demo.example/demo/avatars/photos/sofia-rossi.png",
        country: "it",
        createdAt: stamp,
        roleId: null,
      ...
      },
      {
        ...
        avatarUrl: "https://demo.example/demo/avatars/photos/elena-hoffmann.png",
        country: "de",
        createdAt: stamp,
    ...
    contacts: Array.from({ length: 30 }, (_, index) => ({
      ...
      lastName: "Example",
      avatarUrl: `https://demo.example${SYNTHETIC_CONTACT_AVATAR_PATHS[index]}`,
      notes: null,
      identifiers: [
        {
...
describe("synthetic audit-log fixtures", () => {
  it("builds the current-contract 200-row lifecycle with a creation event for every contact", () => {
    ...
    expect(fixtures.at(-1)?.createdAt).toEqual(new Date("2026-03-28T12:30:00.000Z"));
    expect(
      buildSyntheticAuditLogFixtures({
        companyId: SEED_IDS.company,
        primaryUserId: SEED_IDS.user,
    ...
    expect(roleCreations.map(auditPayload)).toEqual([
      ...
      expect.objectContaining({ name: "Customer Success", isSystemRole: false }),
    ]);
    expect(
