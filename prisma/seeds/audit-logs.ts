...
export const SYNTHETIC_AUDIT_LOG_COUNT = 200;
...
export function buildSyntheticAuditLogFixtures(args: {
  ...
  for (const task of snapshot.tasks) {
    const initialTask = creationState(task, { name: previousTaskName(task.name) });
...
async function loadSyntheticAuditSnapshot(
  ...
  const [
    ...
    prisma.user.findMany({
      where: {
        id: { in: [context.ids.user, context.ids.pendingUser, context.ids.activeUser] },
    ...
    prisma.userRole.findMany({
      where: {
        id: { in: [context.ids.salesManagerRole, context.ids.customerSuccessRole] },
        companyId: context.ids.company,
      },
      orderBy: { id: "asc" },
      select: {
