...
import { fixtureId, publicAssetUrl } from "../helpers";
...
function personAttendee(personKey: PersonKey, provider: MessagingProvider, baseUrl: string): DemoAttendee {
  ...
  return {
    ...
    pictureUrl: publicAssetUrl(baseUrl, person.avatarPath),
    profileUrl: person.profileUrl,
  };
}

function selfAttendee(provider: MessagingProvider, seedUserEmail: string, baseUrl: string): DemoAttendee {
  const identifier =
  ...
  return {
    ...
    pictureUrl: publicAssetUrl(baseUrl, SYNTHETIC_AVATAR_PATHS.maxBergmann),
...
export async function seedDemoMessagingFixtures(prisma: PrismaClient, context: SeedContext): Promise<void> {
  ...
  for (const account of accounts) {
    const data = {
      ...
      shared: false,
      syncing: false,
      ownerAvatarUrl: publicAssetUrl(context.baseUrl, SYNTHETIC_AVATAR_PATHS.maxBergmann),
  ...
  for (const [index, channel] of channelPeople.entries()) {
    const attendee = personAttendee(channel.key, channel.provider, context.baseUrl);
  ...
  for (const [threadIndex, fixture] of threads.entries()) {
    ...
    const self = selfAttendee(provider, context.seedUserEmail, context.baseUrl);
    const counterparts = fixture.participants.map((key) => personAttendee(key, provider, context.baseUrl));
    ...
    for (const [localMessageIndex, message] of fixture.messages.entries()) {
      ...
      const sender = message.sender === "self" ? self : personAttendee(message.sender, provider, context.baseUrl);
      ...
      const reactionSender = message.reaction
        ? message.reaction.sender === "self"
          ? self
          : personAttendee(message.reaction.sender, provider, context.baseUrl)
        : null;
