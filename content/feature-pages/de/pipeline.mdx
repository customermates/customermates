...
description: "Vertriebspipeline verwalten mit Drag-and-Drop Kanban-Boards, individuellen Deal-Phasen und Dashboard-Diagrammen. Externe KI-Clients aktualisieren unterstützte Felder über MCP; die verwaltete Datenbank läuft in einer EU-Region."
...
visualSection:
  ...
  description: Max Bergmann zieht Digital Customer Platform von Offen in Richtung Gewonnen, während Dealwert und gewichteter Wert sichtbar bleiben.
  labels:
    focal: Deal
    semantic:
      - Dealwert
...
**Visuelles Kanban-Board.** Ihre Pipeline wird als Drag-and-Drop-Kanban dargestellt, bei dem jede Spalte eine Deal-Phase repräsentiert. Karten zeigen die für diese Ansicht konfigurierten Felder; die vorbereitete Demo enthält Deal-Name, Status, Gesamt- und gewichteten Wert, Menge, Projektzeitraum, Organisation, Services und Besitzer. Einen Deal in die nächste Phase zu bewegen ist so einfach wie das Ziehen der Karte in die nächste Spalte.
...
**Deal-Karten mit Kontext.** Wählen Sie, welche Felder auf jeder Deal-Karte erscheinen, damit das Board schnelle Entscheidungen unterstützt, ohne für jeden Workspace dasselbe Schema zu behaupten. Die vorbereitete Demo zeigt Status, Gesamt- und gewichteten Wert, Menge, Projektzeitraum, verknüpfte Organisation, Services und Besitzer; öffnen Sie den Datensatz für das vollständige Bild.
...
Die öffentliche Demo dient zum Ansehen. Die vorbereitete Deal-Ansicht öffnet sich als nach Status gruppiertes Kanban, damit Sie echte Karten, Dealwerte, gewichtete Werte und Spaltensummen prüfen können, ohne einen Demo-Drag als gespeicherte Änderung zu verstehen.

<ProductDemo id="pipeline-deal-board-demo" surface="deals" title="Live-Demo des Customermates Deal-Boards" />

## Richten Sie Ihre Pipeline in drei Schritten ein

<Steps>
  <Step id="pipeline-setup-status" title="Statusoptionen definieren">
    Verwenden Sie eine Einfachauswahl-Spalte für den Status, deren Optionen zu
    Ihrem tatsächlichen Vertriebsprozess passen.
  </Step>
  <Step id="pipeline-setup-deals" title="Relevante Deals anlegen">
    Geben Sie jedem Deal einen eindeutigen Namen, den aktuellen Wert, einen
    Besitzer und einen Status, damit das Board sofort nützlichen Kontext zeigt.
  </Step>
  <Step id="pipeline-setup-board" title="Im Board arbeiten">
    Wechseln Sie die Deal-Liste in die Board-Ansicht, gruppieren Sie nach Status
    und prüfen oder verschieben Sie eine Karte, sobald sich ihr Zustand ändert.
  </Step>
</Steps>
...
<FeaturePoints>
  <FeaturePoint id="pipeline-open-source" icon="Code2" title="Open-Source-Kern">
    Kern-CRM-Anwendung und Pipeline-Code unter AGPL sind einsehbar.
    Pipeline-Phasen bleiben kundendefinierte Felder; Customermates enthält keinen
    versteckten Prognose- oder Scoring-Algorithmus, während Managed-Cloud-Funktionen
    und Anbieter separat bleiben.
  </FeaturePoint>
  <FeaturePoint id="pipeline-eu-region" icon="ShieldCheck" title="Dokumentierte Hosting-Grenzen">
    Die verwaltete Datenbank läuft in einer EU-Region. Anwendungshosting und
    weitere Anbieter, darunter US-Anbieter, können Daten andernorts verarbeiten;
    diese Bereitstellungsangabe ist keine DSGVO- oder andere
    Compliance-Zertifizierung.
  </FeaturePoint>
  <FeaturePoint id="pipeline-pricing" icon="DollarSign" title="Einfache, faire Preisgestaltung">
    Pipeline-Management ist ab [[commercial.price.starter.monthly]] pro Nutzer
    und Monat enthalten. Board- und Tabellenansichten, individuelle Phasen,
    Deal-Karten, Dashboard-Diagramme und MCP-Zugang für externe KI-Clients sind
    in jeder Stufe enthalten.
  </FeaturePoint>
  <FeaturePoint id="pipeline-workflows" icon="Zap" title="Separate n8n-Workflows">
    Verbinden Sie eine selbst betriebene oder abonnierte n8n-Instanz über REST,
    Webhooks, MCP oder den Community-Node. Scheduler, Konnektorkatalog, Laufzeit,
    Tarife, Limits, Logs, Anbieterzustellung und Kosten bleiben extern.
  </FeaturePoint>
  <FeaturePoint id="pipeline-self-hosting" icon="Server" title="Self-hostbare Anwendung und Datenbank">
    Betreiben Sie die Customermates Anwendung und PostgreSQL-Datenbank auf eigener
    Infrastruktur. Externe KI- und Workflow-Anbieter erhalten weiterhin alle von
    Ihrer Konfiguration gesendeten Daten; Self-Hosting ist keine Datenresidenz-
