---
title: "CRM With API: REST, Webhooks & Dev Tools"
description: "CRM with API featuring REST endpoints, webhooks, OpenAPI spec, and n8n automation. Full CRUD access, API keys, and SDKs for custom integrations."
featureName: CRM API

hero:
  title: A CRM API Built for Developers Who Ship Fast
  description: Access every piece of CRM data through a clean REST API. Build custom integrations, sync data across systems, and automate workflows with webhooks, API keys, OpenAPI spec, and n8n. A native MCP endpoint also lets AI agents like Claude, ChatGPT, and Gemini read and write the CRM directly. Included in every tier, from EUR 12 per user per month.
...
Customermates provides a comprehensive REST API that gives developers full programmatic access to all CRM data and operations. API access is included in every pricing tier, starting at EUR 12 per user per month, with no separate developer tier.
...
**Full CRUD operations across all entities.** Create, read, update, and delete records for every CRM entity: contacts, organizations, deals, services, and tasks, including their custom fields. Every operation available in the user interface is available through the API, ensuring feature parity.
...
**Native MCP endpoint for AI agents.** Beyond REST, Customermates exposes a native Model Context Protocol (MCP) endpoint at `https://customermates.com/api/v1/mcp`, authenticated with `x-api-key`. AI agents such as Claude, ChatGPT, and Gemini connect to it and operate the CRM directly. 46 tools — 44 across ten toolsets plus two always-on research connectors let them read and write records, move deal stages, and draft follow-ups, and, on a messaging tier, read and send across the unified inbox. Narrow the exposed tools per client with the `?toolsets=` parameter.
...
**AI agents operate your CRM.** The native MCP endpoint turns Customermates into a system AI agents can run. Point Claude, ChatGPT, or Gemini at it and they handle the data entry: creating records, updating deal stages, and drafting follow-ups through the same permissioned API your own code uses. The pipeline stays current without anyone typing into forms.
...
**Automate beyond the UI with code or n8n.** While the n8n engine handles many automation scenarios visually, the API enables custom automation logic for scenarios that require code. Use n8n for no-code workflows and the API for custom scripts. Both approaches work together seamlessly.
...
**Simple, fair pricing.** API access is included in every plan, starting at EUR 12 per user per month. No separate API tier, no per-request charges, no premium developer plans. Every paying user gets full API access.
