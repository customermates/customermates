...
hero:
  ...
  description: Access every piece of CRM data through a clean REST API. Build custom integrations, sync data across systems, and automate workflows with webhooks, API keys, OpenAPI spec, and n8n. All included at EUR 7 per user per month.
...
Customermates provides a comprehensive REST API that gives developers full programmatic access to all CRM data and operations. API access is included at EUR 7 per user per month with no separate developer tier.
...
**Simple, fair pricing.** API access is included in the flat EUR 7 per user per month price. No separate API tier, no per-request charges, no premium developer plans. Every paying user gets full API access.
