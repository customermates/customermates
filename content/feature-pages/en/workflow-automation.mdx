...
**AI agents alongside your workflows.** n8n handles scheduled, rule-based automation. On top of it, Customermates exposes a native MCP integration with 46 tools, so Claude, ChatGPT, and Gemini can operate the CRM directly: reading and writing records, moving deal stages, summarizing meeting notes, classifying incoming leads with natural language, generating personalized email drafts, and extracting key information from unstructured data. Deterministic workflows and AI agents cover the two halves of automation that rule-based systems alone cannot.
...
|---------|--------------|---------|------------|-----------|
| Price/user/month | From €12 | $90+ (workflows) | $25+ | $49+ (automations) |
...
**No per-automation pricing.** Some CRM providers charge per automation, per execution, or per integration. Customermates works with n8n automation from the €12 per user per month Starter [tier](/pricing). Build as many workflows as you need. Run them as often as required. Connect to as many external services as your business demands. The price for automation itself does not change as you scale usage.
...
Not on our side. From the €12/user/month Starter tier you get unlimited webhook events, unlimited REST calls, and the MCP endpoint, with no per-workflow, per-run, or per-integration charges. n8n and its 400+ integrations are a separate product you self-host or subscribe to, so that cost is yours. See full [pricing details](/pricing).
...
[Sign up for Customermates](/auth/signup) and start automating your CRM workflows today. No credit card required for your first seven days. All [sales automation](/features/sales-automation), [cloud CRM](/features/cloud-crm), and workflow features are available from day one.
