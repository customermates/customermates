...
description: "Open-source sales pipeline management software with drag-and-drop kanban boards, custom deal stages, and dashboard charts. AI agents keep every stage current. EU-hosted."
featureName: Sales Pipeline

hero:
  title: A Sales Pipeline That Shows You Exactly Where Every Deal Stands
  description: Visualize your entire pipeline with drag-and-drop kanban boards. AI agents move deal stages and keep records current, so the board tells the truth. Chart pipeline value on your dashboard and spot bottlenecks before they cost you revenue.
...
Customermates provides a visual pipeline with kanban boards, customizable stages, and dashboard charts. The pipeline is a board view of your deals grouped by a single-select stage column, and the same AI agents that run the rest of the CRM keep it current so it reflects reality.

**AI agents keep the pipeline honest.** Through the native MCP integration (45 tools), AI agents like Claude, ChatGPT, and Gemini move deals between stages, update close dates, and draft follow-ups directly in the CRM. Reps stop avoiding the board, so the stages show where deals actually stand instead of where someone last remembered to drag them.
...
**Simple, fair pricing.** Pipeline management is included from EUR 12 per user per month (Starter tier). Board and table views, custom stages, deal cards, dashboard charts, and native AI/MCP access are included at every tier. No premium tier required for advanced pipeline capabilities.
