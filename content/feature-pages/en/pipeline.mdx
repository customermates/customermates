...
**Board and table views.** Switch each deal list between a board view and a table view, and group either one by a stage column, an owner, a linked record, or a date. To keep new business, renewals, or separate product lines apart, save each as its own view and switch between them from the chips above the list.

**Saved views your team shares.** A view remembers the whole list: its filters, its sorting, its grouping, its layout, and its columns. Name it, share it with the workspace, and everyone opens the same pipeline from the same link. Changing a shared view for yourself never changes it for anyone else until you save it.
