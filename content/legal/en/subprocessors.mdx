...
| **Forward Email**, Forward Email, LLC, 16192 Coastal Hwy, Lewes, DE 19958, USA | Email infrastructure: message bodies and attachments are forwarded in memory; 4xx/5xx failed-delivery logs containing error, envelope, header, and sending-server data may be retained for seven days. | USA. The published DPA begins only when a Cover Page is agreed and the agreement is accepted. No Cover Page has been agreed for our account, so that conjunctive commencement condition is unmet. The Standard Contractual Clause annexes are also in the unavailable Cover Page. There is therefore neither an effective Art. 28 agreement nor a completed Chapter V mechanism for this processing; encryption and limited retention do not cure those gaps. |
| **Sentry**, Functional Software, Inc. (d/b/a Sentry), 45 Fremont Street, 8th Floor, San Francisco, CA 94105, USA (EU representative: Sentry Software Netherlands B.V., Amsterdam) | Error and performance monitoring across browser, server, and background workers. Sentry acts as processor for error-event and other service data; it acts as an independent controller for its own account, profile, device, usage, and support data. | Error-event data stored in the EU (Frankfurt); account, authentication, integration, audit-log, and support data stored in the USA. EU-US Data Privacy Framework certification and EU Standard Contractual Clauses. |
| **Lemon Squeezy**, Sold through Link, LLC (formerly Lemon Squeezy LLC), 222 South Main Street, Suite 500, Salt Lake City, UT 84101, USA | Not a subprocessor for self-service purchases: it acts as Merchant of Record and independent controller under its own terms. Its affiliate script loads globally and reads `document.cookie` on every execution. If it finds an existing `ls_aff_ref` cookie or an `aff` parameter, it then performs pseudonymous browser fingerprinting, may set the cookie, and sends attribution data to Lemon Squeezy. Its role for that tracking has not been established. | USA. For self-service purchases, its own privacy policy governs. The contractual role and transfer mechanism for affiliate tracking and the cookie duration remain unresolved. Whether § 25(2) TDDDG applies requires qualified legal review; otherwise consent under § 25(1) TDDDG is required. |
