...
An unexpired Allow choice remains valid until you withdraw it or until we change what this notice asks your permission for, and a later eligible visit is stored without presenting the choice again. If we change what your Allow covers, your earlier decision is treated as if it had not been given: the stored identifiers are no longer used and you are asked again. A correction that does not change what you agreed to, such as recording that a report has since been submitted or that a platform's conversion interface now exists, leaves your decision in place. Because each advertising platform matches a conversion only against the click it issued, a later click from the same platform replaces the earlier one for that platform; identifiers held for other platforms are unaffected.
...
Reporting is not automatic, and it has not begun. Reporting would mean a person at Customermates generating an export from the records described above and submitting it to the platform's own conversion-import interface. Customermates can produce that export for Google Ads, in the format Google's conversion importer accepts. For ChatGPT Ads, Reddit Ads and LinkedIn Ads it cannot: those platforms accept conversions only through programming interfaces Customermates has not built. No export has been submitted to any of the four, no advertising platform has received anything from this feature, and no advertisement of ours has run. Identifiers captured now are deleted at the end of their retention period unless a report is made before then. Your Allow covers the reporting described here for all four platforms named below, including those whose interface does not exist yet, so building the remaining interfaces changes nothing about what you agreed to and you are not asked again for it. If we ever want to report to an advertising platform not named below, or to send more than the report described here, we ask you for your decision again first.

A report will contain the identifier the platform itself issued, the name of the result ("Customermates signup" or "Customermates paid"), the time of the result, a deduplication reference, and the two consent signals Google requires with each imported conversion ("Ad User Data", set to granted, and "Ad Personalization", set to denied because this notice does not ask for personalised advertising). The deduplication reference is a one-way cryptographic value that lets the platform ignore a repeated upload; it does not reveal your workspace and does not let the platform connect the signup and the paid result to each other. The click identifier is itself personal data: the platform can link it to the advertisement it served you. **Beyond that identifier, no directly identifying data is transmitted: no name, no email address, no hashed email address, no telephone number, no IP address, no user agent and no monetary amount.**

No advertising platform has received anything from this feature yet. The companies that will receive a report are Google LLC for Google Ads, OpenAI for ChatGPT Ads, Reddit, Inc. for Reddit Ads and LinkedIn Corporation for LinkedIn Ads. Each of them acts as an independent controller for its own advertising services, not as a processor for Customermates, so each decides for itself what it does with a conversion it receives; no Art. 28 instruction chain applies to them. Transfers outside the EEA rest on the EU-US Data Privacy Framework where the recipient is certified under it, which covers Google, Reddit and LinkedIn, and on standard contractual clauses together with a transfer impact assessment for OpenAI, which is not certified under that framework. We keep standard contractual clauses in place as a fallback for the certified recipients as well. None of the four appears yet at [Subprocessors and Other Recipients](/subprocessors), which lists current recipients; each is added there when the first report is submitted to it.
