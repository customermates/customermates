...
_Version 2026-08-06, last updated 6 August 2026_
...
Although the database itself is operated in Frankfurt am Main, Neon, LLC is established in the United States and is an affiliate of Databricks, Inc. Access to the data from the United States, in particular for the administration, maintenance, and support of the service, can therefore not be ruled out, and Neon engages subprocessors established in the United States. According to Neon's subprocessor list as of 16 April 2026, these are Salesforce, Grafana, Amazon Web Services, and Microsoft Azure, each located in the United States; the current list is available at https://neon.com/subprocessors. For these transfers, Databricks relies on the standard contractual clauses of the EU Commission of 2021 (Implementing Decision (EU) 2021/914) pursuant to Art. 46 GDPR.
...
For incoming email to our own business addresses under `@customermates.com`, we use Forward Email, LLC, 16192 Coastal Hwy, Lewes, DE 19958, United States of America (https://forwardemail.net) as our final hosted operator mailbox. Messages are received and stored by Forward Email; no separate downstream mailbox provider is used. The service is not used to connect customer mailboxes, messaging accounts, or calendars and does not ordinarily receive CRM records. For hosted mailbox storage, Forward Email states that it stores mailbox data in encrypted SQLite databases on LUKS-encrypted disks and stores encrypted backups on Cloudflare R2. It states that one encrypted backup is made daily for each mailbox, is replaced by the next backup containing changed data, and is removed when the alias is deleted. When a human reply is sent from that mailbox, Forward Email also processes the outgoing message through its SMTP service. The provider states that outbound SMTP emails are stored for approximately 30 days. After successful delivery or permanent failure, the message body is purged by default, although body retention may be configured from zero to 30 days; remaining delivery data may persist for the approximately 30-day period. For failed deliveries with a 4xx or 5xx status, it retains error logs for seven days; those logs may contain the SMTP error, envelope data, message headers, and the sending server's IP address or hostname.

Forward Email's service may process sender and recipient addresses, message content, attachments, headers, communication metadata, and related account, domain, alias, and security data. This is ordinarily controller-side business correspondence. If a customer includes personal data governed by our Data Processing Agreement in a support or feedback request, Forward Email also stores and otherwise processes that limited fragment within our processor chain.

Forward Email processes this data on our behalf. Its published Terms, GDPR page, and FAQ state that its standard data processing agreement is accepted automatically with the Terms and requires no separate signature. The published data processing agreement itself, however, defines the agreement as its Standard Terms together with a Cover Page and states that its term starts only when the provider and customer agree to that Cover Page and sign or electronically accept the agreement. We have not agreed such a Cover Page. Because these published statements conflict, we do not rely on them as establishing an effective agreement pursuant to Art. 28 GDPR for our account. For controller-side correspondence, the legal basis for our processing is Art. 6(1)(f) GDPR (legitimate interest in a secure and reliable email infrastructure) and, where necessary for a contractual relationship, Art. 6(1)(b) GDPR. Where Forward Email processes a fragment covered by our Data Processing Agreement within our processor chain, the customer determines the legal basis and we process that fragment on the customer's documented instructions under that agreement.

Forward Email is established in the USA and its published infrastructure chain includes providers outside the EEA, so personal data may be processed in a third country. The provider's published agreement incorporates the EU Standard Contractual Clauses, but the required annexes are set out in a Cover Page that has not been agreed for our account. We therefore do not rely on those clauses as a completed transfer mechanism. Encryption of the mailbox database, storage disks, and backups, together with seven-day retention of failed-delivery logs, reduces exposure, but these technical measures do not replace an effective Art. 28 agreement or a transfer mechanism under Chapter V GDPR.

Further information on data protection at Forward Email can be found at: https://forwardemail.net/en/privacy

### Contractual status

For our account, we cannot substantiate either an effective Art. 28 data processing agreement or a completed Chapter V transfer mechanism with Forward Email. The current processing therefore has unresolved contractual and transfer gaps that require remediation; the technical measures described above do not cure them.
...
Incoming email to our own `@customermates.com` business addresses is received and stored in our hosted operator mailbox at Forward Email. Resend sends the automated transactional and account messages generated by the application; ordinary human replies are sent from the Forward Email mailbox. No separate downstream mailbox provider is used. Resend's processing is governed by a data processing agreement pursuant to Art. 28 GDPR. Forward Email's unresolved contract and transfer status is described in section 2; we do not represent that an effective Art. 28 agreement currently applies to that processing.
...
Before the assistant is made available, we will conclude a data processing agreement with OpenAI pursuant to Art. 28 GDPR. OpenAI is already listed in advance at [Subprocessors](/subprocessors) under the notice procedure set out in our [Data Processing Agreement](/dpa), marked as planned and not yet in use. According to OpenAI, data transmitted through its API is not used to train its own models. Further information:
