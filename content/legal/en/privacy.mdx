...
_Version 2026-09-02, last updated 2 September 2026_
...
An unexpired Allow choice remains valid until you withdraw it or until we change this notice, and a later eligible visit is stored without presenting the choice again. If we change this notice, your earlier decision is treated as if it had not been given: the stored identifiers are no longer used and you are asked again. Because each advertising platform matches a conversion only against the click it issued, a later click from the same platform replaces the earlier one for that platform; identifiers held for other platforms are unaffected.

If the visitor creates a new managed-cloud workspace before expiry, the retained identifiers and those timestamps are copied once, as part of the same registration transaction, to that workspace and its initial owner. They are not copied to invited users or users joining an existing workspace and are not overwritten on later sign-ins. Customermates also records that the workspace was created, and later that it became a paying subscription. The purpose of all of this is to learn which of our advertisements bring people to Customermates, so that we can decide where to spend our advertising budget. We do not build usage profiles, do not combine this data with data from other websites, and do not use it to target advertising at you.

Reporting is not automatic. A person at Customermates generates an export from the records described above and submits it to the platform's own conversion-import interface; today this is done for Google only, through a spreadsheet in a Customermates Google account that Google Ads reads on a schedule. No reporting route to ChatGPT Ads, Reddit Ads or LinkedIn Ads exists, and nothing has been transmitted to those platforms; identifiers captured for them are deleted at the end of their retention period without ever being sent. Before we begin reporting to a platform we do not report to today, we update this notice and ask you for your decision again. A report contains the identifier the platform itself issued, the name of the result ("Customermates signup" or "Customermates paid"), the time of the result, a deduplication reference, and the two consent signals Google requires with each imported conversion ("Ad User Data", set to granted, and "Ad Personalization", set to denied because this notice does not ask for personalised advertising). The deduplication reference is a one-way cryptographic value that lets the platform ignore a repeated upload; it does not reveal your workspace and does not let the platform connect the signup and the paid result to each other. The click identifier is itself personal data: the platform can link it to the advertisement it served you. **Beyond that identifier, no directly identifying data is transmitted: no name, no email address, no hashed email address, no telephone number, no IP address, no user agent and no monetary amount.** Today the only recipient is Google Ireland Limited, which acts as an independent controller for its own advertising services and not as a processor for Customermates. Google Ireland Limited transfers the data onward to Google LLC in the United States, which participates in the EU-US Data Privacy Framework; the participant record is available at https://www.dataprivacyframework.gov/list. Before ChatGPT Ads, Reddit Ads or LinkedIn Ads become recipients we will name the receiving companies and their transfer safeguards here and ask for your decision again. Google's own information about how it uses this data is available at https://business.safety.google/privacy/. The current recipients are also listed at [Subprocessors and Other Recipients](/subprocessors).

Choosing Allow gives two separate consents in one action: one under § 25 para. 1 TDDDG to storing the cookie described above on your device, and one under Art. 6 para. 1 lit. a GDPR to the subsequent processing, including the transmission described above. If you decline, the same cookie is stored for 90 days, but it then holds only the preference, decision time, notice version and expiry, and no identifier at all. You can withdraw consent at any time through "Privacy choices" in the footer, the same control in the same place as the choice you were originally offered. Withdrawing immediately removes the identifiers from the cookie. If a copy was already attached to a workspace, sign in first: the same control then also deletes the workspace copy, which Art. 17 para. 1 lit. b GDPR requires us to do. Withdrawal does not affect the lawfulness of the processing carried out before it (Art. 7 para. 3 sentence 3 GDPR), and a report already submitted to a platform cannot be recalled. Expired identifiers are removed by a daily automated deletion run. The record that a workspace was created, and later that it became a paying subscription, contains no advertising identifier; it is kept for as long as the workspace exists and is deleted with it. No advertising pixel, tag, measurement SDK, Customer Match, enhanced-conversions transfer or advanced matching is active.
