...
_Version 2026-08-26, last updated 26 August 2026_
...
We use Vercel Web Analytics, a usage measurement service of Vercel Inc., 440 N Barranca Ave #4133, Covina, CA 91723, USA (hereinafter "Vercel"). On the managed cloud service, the analytics component loads on website and application pages, including pages available after login, only after the analytics category is accepted. It is not loaded by the upstream application in self-hosted mode.
...
The use of Vercel Web Analytics is based on your consent (Art. 6 para. 1 lit. a GDPR). You may withdraw it at any time through the privacy choices control; withdrawal prevents future loading after the page reloads. According to Vercel, this analytics service does not use cookies or store data on or access data from your device for analytics purposes.
...
The managed Customermates service does not currently load or initialise that affiliate script, and the upstream application does not load it in self-hosted mode. Customermates therefore does not use the script to read an `ls_aff_ref` cookie, create a browser fingerprint, set an affiliate cookie, or send affiliate-attribution data. The integration remains disabled because Lemon Squeezy's contractual role, the applicable transfer mechanism, and the store-specific cookie duration have not yet been verified for our account. It may be enabled only after those questions and the required consent design have been reviewed. An independent self-host operator controls its own deployment and is responsible for any integration it adds.
