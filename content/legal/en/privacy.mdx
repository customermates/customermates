...
{/* TODO: confirm whether Vercel Web Analytics requires consent under section 25 TDDDG and align with the consent mechanism once one exists. */}
...
A list of the subprocessors we engage is available at [Subprocessors](/subprocessors), and further information on the data processing agreement is available at [Data Processing Agreement](/dpa). {/* BLOCKER: execute the Unipile Art. 28 DPA before publishing; this section presumes it is in force. */}
...
Connected-account data is processed at Unipile and is additionally stored in Customermates' own database (see section 2). Unipile states that connected-account data is stored in the EU (France). {/* TODO: vendor-stated; confirm storage locations and any non-EEA transfer (Unipile's subprocessor list includes US Stripe) against the executed DPA + current subprocessor list before publishing. */}
