...
_Version 2026-08-31, last updated 31 August 2026_
...
You have the right to receive information about the origin, recipient, and purpose of your stored personal data at any time, free of charge. You also have the right to request the correction or deletion of this data. If you have given consent to data processing, you can revoke this consent at any time for the future. Furthermore, you have the right to request the restriction of the processing of your personal data under certain circumstances. You also have the right to lodge a complaint with the competent supervisory authority. You can contact us at any time regarding this and other questions about data protection. Analysis and third-party tools: use of the managed public website is measured in aggregated form. On managed-cloud public content pages, affiliate referrals are attributed using Lemon Squeezy's standard script. Google advertising and analytics tags remain disabled; after consent, Customermates can retain one Google Ads click identifier for first-party attribution. Details are set out below.
...
### First-party Google Ads click attribution

When a managed-cloud public content page or the contact page is opened with a valid Google Ads click identifier (`gclid`, `gbraid` or `wbraid`) and no attribution decision is stored, Customermates offers an optional attribution choice. If you consent, a signed, first-party, HTTP-only cookie stores exactly one identifier with its identifier kind, capture time, consent decision time and expiry. It does not store UTM parameters, referrers, landing paths, form contents or CRM records. The cookie and raw identifier expire no later than 90 days after the decision.

While the choice remains open, the identifier and a technical pending marker are carried in the address of Customermates pages you navigate to so the choice survives navigation and reloads. The application recognises this marked pending state for 24 hours, with up to five minutes of device-clock tolerance. This does not create a Customermates attribution cookie or user record before you choose Allow. Browser history may retain an earlier marked address until it is revisited; after a decision, expiry, or an invalid marker, the application removes both values from the current address and from an earlier such history entry when you revisit it.

An unexpired Allow choice remains valid until withdrawal. If that cookie currently contains no click identifier, one identifier from a later eligible Google Ads visit may be stored without presenting the choice again; an existing identifier is never replaced.

If the visitor creates a new managed-cloud workspace before expiry, the identifier and those timestamps are copied once, as part of the same registration transaction, to the initial owner's user record. They are not copied to invited users or users joining an existing workspace and are not overwritten on later sign-ins. Existing registration, onboarding and subscription fields may be reviewed internally to assess the limited paid-search experiment. Customermates does not create a separate behavioural lifecycle profile for this purpose.

The legal basis for the cookie and linked first-party processing is consent under § 25 para. 1 TDDDG and Art. 6 para. 1 lit. a GDPR. If you decline, we store only the preference, decision time and expiry and do not store a click identifier. You can withdraw consent through “Privacy choices” in the footer; this removes the identifier from the cookie and, when you are signed in, from your user record. Expired identifiers are removed by the daily maintenance process. No Google tag, Google conversion action, conversion upload, Customer Match or enhanced-conversions transfer is active for this initial storage-only setup.
...
- tool descriptions, tool inputs and results, approval and confirmation state, UI-command results, and generated output; and
...
Mate does not automatically upload the entire workspace. A model request contains the prompt and recent conversation together with the context and tool results needed for the requested task. Requests are routed through **Vercel AI Gateway**, operated by Vercel Inc., to **OpenAI** as the selected model provider. Vercel and OpenAI therefore receive the parts of a request necessary to generate the response. The current provider chain and transfer information are listed at [Subprocessors](/subprocessors).

### Legal basis and roles

For CRM and connected-account content, including personal data concerning the customer's contacts and correspondents, the customer is the controller and Customermates processes that content on the customer's documented instructions as a processor pursuant to Art. 28 GDPR. Sending a Mate request or confirming an action is such an instruction; it is not consent within the meaning of Art. 6(1)(a) GDPR. The customer remains responsible for having a legal basis for the content and for deciding whether it may be included in a Mate request.
...
Vercel's AI Gateway terms treat third-party model providers as non-Vercel services and make their provider terms applicable. Vercel's current Notices and License Information links OpenAI's Terms of Use for this route. The public product source does not establish that an account-specific training opt-out, business/API data-processing configuration, or Zero Data Retention setting applies. Customermates therefore does not promise that provider-side training is disabled, that a particular provider retention period applies, or that prompts and outputs are deleted immediately. Users must not send sensitive personal information through Mate, including health information, financial-account information, or government-issued identifiers, and customers must not make CRM or connected-account context containing such information available to a Mate request.

Use of Mate can involve processing in the United States and other countries used by the providers and their subprocessors. Vercel's data processing addendum incorporates the 2021 EU Standard Contractual Clauses for restricted transfers. The terms applicable to the OpenAI route, the recipient roles, and the known limitations are set out at [Subprocessors](/subprocessors). Further information:
