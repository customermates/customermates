...
For the provision and forwarding of our email communication, we use the services of Forward Email, LLC, 16192 Coastal Hwy, Lewes, DE 19958, United States of America (https://forwardemail.net). Forward Email provides the technical infrastructure through which incoming emails are processed and forwarded to our recipient addresses. According to Forward Email, message bodies and attachments are forwarded in memory rather than retained in persistent storage. For failed deliveries with a 4xx or 5xx status, it retains error logs for seven days; those logs may contain the SMTP error, envelope data, message headers, and the sending server's IP address or hostname.
...
Forward Email processes this data on our behalf. Its published data processing agreement states that it starts only when the provider and customer agree to a Cover Page and sign or electronically accept the agreement. We have not agreed such a Cover Page and therefore do not treat the published terms as an effective agreement pursuant to Art. 28 GDPR for our account. The legal basis for our processing is Art. 6(1)(f) GDPR (legitimate interest in a secure and reliable email infrastructure) and, where necessary for a contractual relationship, Art. 6(1)(b) GDPR.

Since Forward Email operates servers in the USA, personal data is transferred to a third country. The provider's published agreement incorporates the EU Standard Contractual Clauses, but the required annexes are set out in a Cover Page that the provider makes available only under its Enterprise offering. We therefore do not rely on those clauses as a completed transfer mechanism. Encryption, in-memory forwarding of message bodies and attachments, and seven-day retention of failed-delivery logs reduce exposure, but these technical measures do not replace an effective Art. 28 agreement or a transfer mechanism under Chapter V GDPR.

Further information on data protection at Forward Email can be found at: https://forwardemail.net/en/privacy

### Contractual status

For our account, there is neither an effective Art. 28 data processing agreement nor a completed Chapter V transfer mechanism with Forward Email. The current processing therefore has unresolved contractual and transfer gaps that require remediation; the technical measures described above do not cure them.
...
Incoming and outgoing e-mail is handled by the providers named in section 2: Forward Email for incoming e-mail and Resend for outgoing e-mail. Resend's processing is governed by a data processing agreement pursuant to Art. 28 GDPR. Forward Email's unresolved contract and transfer status is described in section 2; we do not represent that an effective Art. 28 agreement currently applies to that processing.
...
Affiliate tracking is used for the **attribution of concluded contracts to referring partners** and the settlement of affiliate commissions. The root script is loaded from Lemon Squeezy on every page of our website and web application, so each load discloses ordinary request metadata, including the browser's IP address, to Lemon Squeezy. On every execution, the script also reads `document.cookie` to look for an existing `ls_aff_ref` cookie.

If neither an existing `ls_aff_ref` cookie nor an `aff` parameter in the page URL is found, the script exits without loading FingerprintJS or sending the affiliate tracking request. Otherwise it incorporates FingerprintJS and reads browser and device signals including canvas, audio, fonts, plugins, storage availability, screen, platform, and hardware characteristics; where the trigger is an `aff` parameter, it can set the `ls_aff_ref` cookie. The cookie duration is controlled by the affiliate-program setting and has not yet been verified for our store.

When affiliate identification is triggered, the script sends the page URL, referrer, affiliate or click reference, and a **pseudonymous visitor identifier** to Lemon Squeezy's affiliate endpoint. For the ordinary request metadata disclosed by the unconditional script load and for subsequent personal-data processing for affiliate attribution, we currently rely on Art. 6(1)(f) GDPR (legitimate interest in administering the affiliate program and settling commissions). Whether the unconditional load is necessary and passes the required balancing test remains subject to qualified legal review. Storage of device information or access to it requires a separate basis under § 25 TDDDG; Art. 6 GDPR does not replace it. Whether the exception in § 25(2) TDDDG applies is unresolved and requires qualified legal review; if it does not, consent under § 25(1) TDDDG and a corresponding technical mechanism are required.
