...
When a managed-cloud public content page or the contact page is opened with exactly one valid identifier and no attribution decision is stored, Customermates offers an optional attribution choice. A page carrying more than one recognised identifier is ignored entirely. If you consent, a signed, first-party, HTTP-only cookie stores at most one identifier per advertising platform, each with its identifier kind, click time, capture time and expiry, alongside your decision, the decision time and the version of this notice. It does not store UTM parameters, referrers, landing paths, form contents or CRM records. Raw identifiers expire 89 days after the click for Google and LinkedIn and 30 days after the click for ChatGPT and Reddit, and never later than 90 days after your decision.

While the choice remains open, the identifier and a technical pending marker are carried in the address of Customermates pages you navigate to so the choice survives navigation and reloads. The application recognises this marked pending state for 24 hours, with up to five minutes of device-clock tolerance. This does not create a Customermates attribution cookie or user record before you choose Allow. The notice does not block the page: you can read and use the site while the choice is open, and nothing is stored until you choose Allow. Browser history may retain an earlier marked address until it is revisited; after a decision, expiry, or an invalid marker, the application removes both values from the current address and from an earlier such history entry when you revisit it.

An unexpired Allow choice remains valid until withdrawal, and a later eligible visit is stored without presenting the choice again. Because each advertising platform matches a conversion only against the click it issued, a later click from the same platform replaces the earlier one for that platform; identifiers held for other platforms are unaffected.

If the visitor creates a new managed-cloud workspace before expiry, the retained identifiers and those timestamps are copied once, as part of the same registration transaction, to that workspace and its initial owner. They are not copied to invited users or users joining an existing workspace and are not overwritten on later sign-ins. Customermates also records that the workspace was created, and later that it became a paying subscription, so that the advertising platform can be told its advertisement led to a result.

Customermates reports those results back to the platform that issued the identifier. Only the identifier the platform itself issued, the kind of result, its time and a deduplication reference are transmitted. **No personal data is transmitted: no name, no email address, no hashed email address, no telephone number, no IP address, no user agent and no monetary amount.** The recipients are Google Ireland Limited and Google LLC, OpenAI Ireland Limited, Reddit, Inc. and LinkedIn Ireland Unlimited Company and LinkedIn Corporation, each acting as an independent controller for its own advertising services. For transfers to the United States, Google LLC, Reddit, Inc. and LinkedIn Corporation participate in the EU-US Data Privacy Framework; transfers to OpenAI are based on the European Commission's standard contractual clauses.

The legal basis for the cookie and the linked first-party processing, including the transmission described above, is your consent under § 25 para. 1 TDDDG and Art. 6 para. 1 lit. a GDPR. If you decline, we store only the preference, decision time, notice version and expiry, and no identifier at all. You can withdraw consent through "Privacy choices" in the footer; this removes the identifiers from the cookie and, when you are signed in, from your workspace. Expired identifiers are removed by the daily maintenance process. No advertising pixel, tag, measurement SDK, Customer Match, enhanced-conversions transfer or advanced matching is active.
