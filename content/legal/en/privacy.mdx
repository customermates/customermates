...
### Forward Email (business email forwarding)

For incoming email to our own business addresses under `@customermates.com`, we use Forward Email, LLC, 16192 Coastal Hwy, Lewes, DE 19958, United States of America (https://forwardemail.net). Forward Email provides the technical infrastructure through which those messages are processed and forwarded to our operator mailbox. It is not used to connect customer mailboxes, messaging accounts, or calendars and does not ordinarily receive CRM records. According to Forward Email, message bodies and attachments are forwarded in memory rather than retained in persistent storage. For failed deliveries with a 4xx or 5xx status, it retains error logs for seven days; those logs may contain the SMTP error, envelope data, message headers, and the sending server's IP address or hostname.

In the course of this forwarding, personal data may pass through Forward Email's servers. This includes sender and recipient addresses, message content, metadata, communication data, and attachments where technically required. This is ordinarily controller-side business correspondence. If a customer includes personal data governed by our Data Processing Agreement in a support or feedback request, Forward Email also processes that limited fragment within our processor chain.
...
Incoming email to our own `@customermates.com` business addresses is forwarded through Forward Email to our operator mailbox. Resend sends the automated transactional and account messages generated by the application; ordinary human replies from the operator mailbox are handled by the mailbox provider selected for that account. Resend's processing is governed by a data processing agreement pursuant to Art. 28 GDPR. Forward Email's unresolved contract and transfer status is described in section 2; we do not represent that an effective Art. 28 agreement currently applies to that processing.
