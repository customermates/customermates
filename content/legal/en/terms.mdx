...
Where the customer uses Mate with CRM or connected-account content, the Provider processes that content on the customer's documented instructions under the same data processing agreement and routes the necessary request context through Vercel AI Gateway to OpenAI. The current provider chain, data categories, retention behavior, and transfer information are described in the [Privacy Policy](/privacy), [Data Processing Agreement](/dpa), and [Subprocessors](/subprocessors).
