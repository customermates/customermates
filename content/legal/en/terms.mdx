...
For the managed service, the Provider sends an automated change notice to every active system administrator at the email address stored for that user. The notice identifies every changed document and version, provides links to the current documents and to revision-specific copies tied to the deployed source-code commit, and prominently states the common objection and acceptance deadline. The amended contractual documents are intended to take effect for the customer 14 calendar days after the first such notice has been successfully sent to one of its active system administrators; notices retried or sent later to another administrator use the same company-wide effective date.
