...
_Version 2026-08-30, last updated 30 August 2026; effective for each Customer when concluded under Section 2_
...
- Mate prompts, recent conversation text, generated output, tool definitions, tool inputs and results, approval and confirmation state, UI-command results, model-round records, model and provider identifiers, timestamps, and the company and user identifiers needed to associate those records with the Customer;
...
For Mate, model requests are routed through Vercel AI Gateway to OpenAI and may be processed in the United States or other countries used by those providers and their subprocessors. Vercel's data processing addendum incorporates the 2021 EU Standard Contractual Clauses for restricted transfers covered by that addendum. Vercel's current Notices and License Information applies OpenAI's Terms of Use to the provider route; the public product source does not establish an account-specific business/API data-processing configuration or transfer mechanism for OpenAI. Before customer-controlled personal data is processed through that route, the Provider must ensure that the required Art. 28 and Chapter V safeguards apply. The current recipient and transfer information, including the absence of a public Zero Data Retention promise for Customermates, is stated at [Subprocessors](/subprocessors).
