...
Das Affiliate-Tracking dient der **Zuordnung abgeschlossener Verträge zu vermittelnden Partnern** und der Abrechnung von Affiliate-Provisionen. Das Root-Skript wird auf jeder Seite unserer Website und Webanwendung von Lemon Squeezy geladen; bei jedem Laden werden daher gewöhnliche Anfragemetadaten einschließlich der IP-Adresse des Browsers an Lemon Squeezy übermittelt. Bei jeder Ausführung liest das Skript außerdem `document.cookie`, um nach einem vorhandenen `ls_aff_ref`-Cookie zu suchen.

Findet das Skript weder ein vorhandenes `ls_aff_ref`-Cookie noch einen `aff`-Parameter in der Seiten-URL, beendet es die Ausführung, ohne FingerprintJS zu laden oder die Affiliate-Tracking-Anfrage zu senden. Andernfalls bindet es FingerprintJS ein und liest Browser- und Gerätesignale aus, darunter Canvas, Audio, Schriftarten, Plugins, Speicherverfügbarkeit, Bildschirm, Plattform und Hardwareeigenschaften; bei Auslösung durch einen `aff`-Parameter kann es das `ls_aff_ref`-Cookie setzen. Die Cookie-Laufzeit wird in den Einstellungen des Affiliate-Programms festgelegt und wurde für unseren Shop noch nicht verifiziert.

Wird die Affiliate-Identifizierung ausgelöst, sendet das Skript die Seiten-URL, den Referrer, die Affiliate- oder Klick-Referenz und eine **pseudonyme Besucherkennung** an den Affiliate-Endpunkt von Lemon Squeezy. Für die durch das bedingungslose Laden des Skripts übermittelten gewöhnlichen Anfragemetadaten und für die anschließende Verarbeitung personenbezogener Daten zur Affiliate-Zuordnung stützen wir uns derzeit auf Art. 6 Abs. 1 lit. f DSGVO (berechtigtes Interesse an der Verwaltung des Affiliate-Programms und der Provisionsabrechnung). Ob das bedingungslose Laden erforderlich ist und der gebotenen Interessenabwägung standhält, bedarf qualifizierter rechtlicher Prüfung. Die Speicherung von Geräteinformationen oder der Zugriff darauf erfordert eine gesonderte Grundlage nach § 25 TDDDG; Art. 6 DSGVO ersetzt diese nicht. Ob die Ausnahme in § 25 Abs. 2 TDDDG greift, ist ungeklärt und bedarf ebenfalls qualifizierter rechtlicher Prüfung; andernfalls sind eine Einwilligung nach § 25 Abs. 1 TDDDG und ein entsprechender technischer Mechanismus erforderlich.
