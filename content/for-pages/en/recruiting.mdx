...
description: "Recruiting CRM for agencies and in-house talent teams. Candidate pipelines, client tracking, automation. Open-source, EU-hosted, from [[commercial.price.starter.monthly]]/user/month."
industryName: "Recruiting"

hero:
  title: "The Open-Source Recruiting CRM for Modern Talent Teams"
  description: "Manage candidates, clients, and job orders in one recruiting CRM. AI agents do the data entry, and a native inbox unifies LinkedIn, email, and WhatsApp. Pipeline-first, agent-native, EU-hosted."
...
### LinkedIn and the unified inbox for sourcing-heavy desks

Recruiting runs on LinkedIn, so it sits at the center of the native unified inbox alongside email, WhatsApp, Instagram, and Telegram (Pro and up). A candidate you find through Sales Navigator or LinkedIn Recruiter goes into the CRM in two clicks with their profile context attached, and every LinkedIn message, email, and WhatsApp thread lands on the candidate record. The same AI agents can read, draft, and send across all of them. This is the single biggest time saver on a sourcing-heavy desk. See [LinkedIn integration](/features/linkedin-integration).

### Task and follow-up management

Every search has its rhythm of follow-ups. [Task management](/features/task-management) keeps candidate and client work in one list with assignees and notes; model follow-up dates with custom date columns. A separately operated workflow can send reminders, while the recruiter focuses on the call itself.

### API and webhooks for custom workflows

The full Customermates [API](/features/api) and [[derived.webhooks.events.total]] webhook events let agencies plug the CRM into their existing job-board posting tools, video interview platforms, payroll software, and invoicing system. The CRM becomes the system of record without ripping out the rest of the stack.
...
Customermates pricing starts at [[commercial.price.starter.monthly]] per user per month for Starter, the core CRM with no messaging inbox. Recruiting runs on the inbox, so most agencies land on Pro instead: the unified inbox for email, LinkedIn, and WhatsApp costs [[commercial.price.pro.monthly]] per user per month. There is a [[commercial.trial.days]]-day free trial on Pro, no credit card required. The full pricing breakdown sits at our [pricing page](/pricing).

A small recruiting agency of five recruiters on Pro runs at [[commercial.price.pro.monthly.seats.5]] per month, a small fraction of a typical placement fee. Self-hosting is free if you bring your own infrastructure.
...
<Faq>
<FaqItem question="Is Customermates a CRM or an ATS?">
...
<FaqItem question={"What does \"agent-native\" mean for a recruiting CRM?"}>
...
<FaqItem question="Is Customermates GDPR-compliant for European recruiting?">
...
<FaqItem question="How long does it take to migrate from another recruiting CRM?">
Customermates has no CSV uploader, bulk-import workflow, or field-mapping screen. Prepare and map source data outside the product, create supported records through the [REST API](/features/api) or an external AI client over MCP, and verify the results in the CRM. Migration time depends on the source data and workflow.
</FaqItem>

<FaqItem question="Can the recruiting CRM track contractor-to-permanent conversions?">
...
<FaqItem question="How does Customermates handle off-limits and confidential candidates?">
Off-limits status can be represented with a custom checkbox field on the contact record, plus a free-text reason and an expiry date. Permissions apply by resource and action with own-versus-all scope; Customermates has no private mandate or candidate-slate access model. For fully confidential mandates, design the process around those boundaries and assess self-hosting plus every connected provider separately.
</FaqItem>

<FaqItem question="Can I build my own custom recruiting CRM instead?">
...
<RelatedPages>
  <RelatedPage href="/for/cleaning-companies" />
  <RelatedPage href="/for/ecommerce" />
