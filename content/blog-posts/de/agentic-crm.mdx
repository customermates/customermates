...
Customermates unterstützt das MCP-Muster nativ. Es stellt [[derived.mcp.tools.total]] MCP-Tools bereit, die alle Entitätstypen abdecken: Kontakte, Unternehmen, Deals, Services, Aufgaben, benutzerdefinierte Felder, Webhooks und Widgets. Neun dieser Tools bedienen einen nativen vereinheitlichten Posteingang, sodass derselbe Agent auch über E-Mail, LinkedIn, WhatsApp, Instagram und Telegram lesen, entwerfen und senden kann, direkt im CRM. Ein über MCP verbundener Agent kann das gesamte Spektrum agentischer CRM-Verhaltensweisen ausführen, ohne proprietäre Abhängigkeiten.
...
Der praktische Vorteil: Sie konfigurieren Absichten einmalig, statt einen wachsenden Baum aus Verzweigungsregeln zu pflegen. Der Agent bewältigt die Ausnahmen.
...
**Webhook-Ereignisse.** Das CRM muss Echtzeit-Ereignisse auslösen, wenn sich Datensätze ändern, damit Agenten auf die Welt reagieren können, ohne Polling zu betreiben. Customermates feuert [[derived.webhooks.events.total]] Webhook-Ereignisse: [[derived.webhooks.events.records]] Datensatz-Ereignisse, die jeden Entitätslebenszyklus abdecken (angelegt, aktualisiert, gelöscht), plus [[derived.webhooks.events.messaging]] Messaging- und Kalender-Ereignisse.
...
Die meisten agentischen CRM-Produkte sind proprietäre SaaS-Lösungen. Das funktioniert, bedeutet aber, dass Ihre Kundendaten auf fremder Infrastruktur liegen und das KI-Verhalten innerhalb der Plattform intransparent ist.
...
Customermates ist [Open Source](/blog/open-source-crm) (AGPL-lizenziert) und selbst hostbar. Die agentischen Verhaltensweisen kommen vom KI-Agenten, den Sie verbinden, nicht von einer gesperrten KI-Schicht innerhalb des Produkts. Das bedeutet: Sie können heute Claude verwenden und morgen das Modell wechseln, ohne Ihr CRM zu migrieren. Die DSGVO-Konformität bleibt vollständig unter Ihrer Kontrolle.

## Einstieg in agentisches CRM

Der schnellste Weg zu agentischem CRM-Verhalten erfordert nicht, Ihren gesamten CRM-Stack zu ersetzen. Er erfordert, Ihre CRM-Daten über MCP für einen KI-Agenten zugänglich zu machen.

Für Customermates:

1. Generieren Sie einen API-Schlüssel im Einstellungsbereich
2. Fügen Sie die MCP-Konfiguration zu Ihrem KI-Client hinzu (Claude Desktop oder ein anderer MCP-kompatibler Host)
3. Bitten Sie den Agenten, eine Aktion auszuführen: "Zeige mir alle Deals, die seit zwei Wochen nicht aktualisiert wurden, und entwirf Follow-ups für jeden"
...
Die [vollständige API-Dokumentation](/features/api) deckt Authentifizierung und verfügbare Tools ab. Die [Preise](/pricing) für Customermates Cloud beginnen bei [[commercial.price.starter.monthly]] pro Nutzer und Monat (Starter-Stufe), mit einer kostenlosen Self-Hosting-Option.
...
**Customermates.** Kein nativer KI-Agent im Produkt. Stattdessen: [[derived.mcp.tools.total]] MCP-Tools, über die jeder externe KI-Agent (Claude, GPT-4o, eigene Modelle) das vollständige CRM bedienen kann, plus ein nativer vereinheitlichter Posteingang (E-Mail, LinkedIn, WhatsApp, Instagram, Telegram), den derselbe Agent lesen und versenden kann. Vorteil: Modellunabhängigkeit, volle Datenkontrolle, DSGVO-freundliches Self-Hosting. Cloud ab [[commercial.price.starter.monthly]]/Nutzer/Monat (Starter), Posteingang ab Pro ([[commercial.price.pro.monthly]]).
...
| Customermates         | Nein (extern) | Ja, nativ         | Ja           | ab [[commercial.price.starter.monthly]]/Nutzer/Monat |
...
<FaqItem question="Was ist der Unterschied zwischen agentischer KI und generativer KI im CRM?">
...
<FaqItem question="Was ist ein KI-Agent für das CRM?">
Ein KI-Modell, das konfiguriert ist, über eine API- oder MCP-Verbindung mit CRM-Daten zu interagieren. Es kann Datensätze lesen, Aktualisierungen schreiben, Aufgaben auslösen und Follow-ups basierend auf von Ihnen definierter Geschäftslogik erstellen.
</FaqItem>

<FaqItem question="Unterstützt Customermates agentische CRM-Workflows?">
Ja. Customermates stellt [[derived.mcp.tools.total]] MCP-Tools bereit, damit jeder MCP-kompatible KI-Agent das vollständige CRM bedienen kann. Webhooks feuern bei jeder Entitätsänderung, damit Agenten in Echtzeit reagieren können.
</FaqItem>

<FaqItem question="Was ist das beste Open-Source agentische CRM?">
...
<FaqItem question="Ist agentisches CRM DSGVO-konform?">
...
<FaqItem question="Wie lange dauert die Implementierung eines agentischen CRM-Setups?">
Für ein Team von fünf bis zehn Personen dauert die technische Einrichtung eines MCP-verbundenen Agenten mit Customermates wenige Stunden. Die eigentliche Arbeit liegt in der Konfiguration: Welche Aktionen darf der Agent autonom ausführen? Ab wann braucht er menschliche Freigabe? Für diese Fragen sollten Sie zwei bis vier Wochen einplanen, bis das Team den richtigen Automationsgrad gefunden hat.
</FaqItem>

<FaqItem question="Ersetzen KI-Agenten Vertriebsmitarbeiter?">
...
<FaqItem question="Welche KI-Modelle funktionieren am besten für CRM-Agenten?">
