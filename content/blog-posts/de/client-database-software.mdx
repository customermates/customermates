...
| **Customermates** | **Open Source, KI-nativ, vereinheitlichter Posteingang, EU-Hosting** | **Ab 12 € / Nutzer / Monat (Cloud)** | **Kostenlos selbst hostbar** | **Ja (Docker)** | **AGPL-3.0** | **Nativer MCP-Server** | **Deutschland (Cloud), eigener Server (Self-Host)** |
...
**Spezifische Funktionen.** Eingebauter MCP-Server mit 46 Tools für direkten Zugriff durch Claude, Codex oder ChatGPT. Der Agent legt Kontakte an, schreibt Notizen, aktualisiert Deals und liest oder sendet Nachrichten im vereinheitlichten Posteingang, ohne dass Sie ins CRM einloggen. Nativer vereinheitlichter Posteingang für E-Mail, LinkedIn, WhatsApp, Instagram und Telegram ab der Pro-Stufe. n8n-Community-Node für visuelle Workflow-Automatisierung. 26 Webhook-Events ab dem Einstiegstarif. Zehn typisierte Custom-Field-Typen (Text, E-Mail, Telefon, Link, Datum, Datumsbereich, Datum und Uhrzeit, Datum-und-Uhrzeit-Bereich, Währung, Single-Select). Self-Hosting via `docker compose up -d` in 30 Minuten produktiv.

**Preisstaffel.** Self-Hosting kostenlos unter AGPL-3.0 via Docker Compose, eigene Infrastruktur. Cloud ab 12 Euro pro Nutzer pro Monat (10 Euro im Jahresplan, Starter-Stufe), in der EU-Region gehostet. Keine Add-on-Aufschläge für Webhooks, API oder MCP, alles im Basistarif (der vereinheitlichte Posteingang startet in der Pro-Stufe für 29 Euro).
