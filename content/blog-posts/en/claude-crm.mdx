...
hero:
  ...
  description: "What people mean by Claude CRM, what Claude can actually do inside a CRM, and the open-source CRM where Claude already operates [[derived.mcp.tools.total]] MCP tools out of the box."
...
| Cost sensitivity     | Cannot justify [[commercial.price.starter.monthly]]/user | Can afford a small subscription |
...
[Customermates](/) is an open-source CRM built MCP-first. It exposes [[derived.mcp.tools.total]] native MCP tools: [[derived.mcp.tools.grouped]] across [[derived.mcp.toolsets.count]] toolsets that cover every entity in the data model, plus [[derived.mcp.tools.alwaysOn]] always-on deep-research connectors.

- **Records ([[derived.mcp.tools.groups.records]]):** filter, count, search, get, create, update, link, unlink, delete, plus note and custom-field edits across contacts, organizations, deals, services, and tasks
- **Messaging ([[derived.mcp.tools.groups.messaging]]):** read, draft, and send across email, LinkedIn, WhatsApp, Instagram, and Telegram
- **Social and Sales Navigator ([[derived.mcp.tools.groups.social]]):** social-post tracking and LinkedIn Sales Navigator search
- **Docs ([[derived.mcp.tools.groups.docs]]):** search and read the product documentation
- **Workspace ([[derived.mcp.tools.groups.workspace]]):** current user and workspace context
- **Admin ([[derived.mcp.tools.groups.admin]]):** roles and company management
- **Custom columns ([[derived.mcp.tools.groups.custom-columns]]):** manage custom fields across ten field types
- **Widgets ([[derived.mcp.tools.groups.widgets]]):** dashboard widgets
- **Webhooks ([[derived.mcp.tools.groups.webhooks]]):** manage webhook subscriptions
- **Support ([[derived.mcp.tools.groups.support]]):** request product support
- **Deep research ([[derived.mcp.tools.alwaysOn]]):** always-on `search` and `fetch` connectors

The messaging toolset is the second half of the story. The same agents that keep records current also operate a native unified inbox, reading, drafting, and sending across email, LinkedIn, WhatsApp, Instagram, and Telegram from one tool surface. Messaging and social tools require a Pro or Business tier; the core record, custom-field, and workspace tools are available from the Starter tier.
...
Claude reads the MCP tool list, identifies `create_contacts`, `manage_record_links`, `create_deals`, and `create_tasks`. It runs them in sequence in one turn, returns the new record IDs, and confirms what it did. The whole interaction takes seconds. No webhook configuration, no Zapier, no admin work.
...
1. **Customermates.** Native MCP support. [[derived.mcp.tools.total]] tools. Open source. Native Claude integration with no separate AI tier.
...
**You want AI in your CRM, your team is 1-30 people, and you do not want a connector tax → native.** Use Customermates. Free to self-host via Docker, [[commercial.price.starter.monthly]] per user per month on EU cloud (Starter tier), open-source AGPL-3.0. Claude operates it directly through [[derived.mcp.tools.total]] MCP tools, and the same agents run the unified inbox across email, LinkedIn, WhatsApp, Instagram, and Telegram on Pro and up.
...
<Faq>
<FaqItem question="What is a Claude CRM?">
...
<FaqItem question="Can Claude build a CRM?">
...
<FaqItem question="Can I make a CRM with Claude?">
...
<FaqItem question="Does HubSpot have a Claude integration?">
...
<FaqItem question="What is the cheapest way to use Claude with a CRM?">
[Customermates](/) is from [[commercial.price.starter.monthly]] per user per month on EU cloud or free to self-host via Docker, with native Claude support through [[derived.mcp.tools.total]] MCP tools. The core record and workspace tools are in every tier from Starter; there is no separate AI tier or connector charge.
</FaqItem>
...
<FaqItem question="What are the top 5 CRMs that work with Claude?">
...
<FaqItem question="Is there an open-source Claude CRM?">
Yes. [Customermates](/) is an open-source CRM under AGPL-3.0 with native Claude support through MCP. It is one of the few that does not require a connector or a separate AI license to access AI features.
</FaqItem>

<FaqItem question="Does Claude work with Salesforce?">
