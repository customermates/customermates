...
Customermates supports the MCP pattern natively. It exposes [[derived.mcp.tools.total]] MCP tools, covering every entity type: contacts, organizations, deals, services, tasks, custom fields, webhooks, and widgets. Nine of those tools operate a native unified inbox, so the same agent can also read, draft, and send across email, LinkedIn, WhatsApp, Instagram, and Telegram from inside the CRM. An agent connected over MCP can perform the full range of agentic CRM behaviors without proprietary lock-in.
...
**Customermates.** Open-source, MCP-first CRM. Rather than building its own AI layer, Customermates exposes [[derived.mcp.tools.total]] MCP tools so any external AI agent (Claude, GPT-4o, custom models) can operate the full CRM autonomously, and it adds a native unified inbox (email, LinkedIn, WhatsApp, Instagram, Telegram) the same agent can read and send from. Free to self-host; cloud from [[commercial.price.starter.monthly]]/user/month (Starter), with the unified inbox on Pro from [[commercial.price.pro.monthly]]. Suited to technical teams that want full control over the AI model and behavior. Compare on the [HubSpot alternative](/compare/hubspot-alternative) or [Pipedrive alternative](/compare/pipedrive-alternative) pages for a direct feature breakdown.
...
**Webhook events.** The CRM must emit real-time events when records change, so agents can react to the world without polling. Customermates fires [[derived.webhooks.events.total]] webhook event types: [[derived.webhooks.events.records]] record events covering every entity lifecycle (created, updated, deleted) plus [[derived.webhooks.events.messaging]] messaging and calendar events.
...
The [full API documentation](/features/api) covers authentication and available tools. [Pricing](/pricing) for Customermates cloud starts at [[commercial.price.starter.monthly]] per user per month (Starter tier), with a free self-hosted option. Sending the drafted follow-ups directly needs Pro at [[commercial.price.pro.monthly]] per user per month, since Starter covers the core CRM without the messaging inbox.
...
<FaqItem question="What is the difference between agentic AI and generative AI in CRM?">
...
<FaqItem question="What is an AI agent for CRM?">
...
<FaqItem question="Does Customermates support agentic CRM workflows?">
Yes. Customermates exposes [[derived.mcp.tools.total]] MCP tools so any MCP-compatible AI agent can operate the full CRM. Webhooks fire on every entity change, allowing agents to react in real time.
</FaqItem>

<FaqItem question="What is the best open source agentic CRM?">
...
<FaqItem question="How do agentic CRM agents handle mistakes?">
...
<FaqItem question="What is multi-agent orchestration in CRM?">
...
<FaqItem question="How does agentic CRM affect sales team structure?">
