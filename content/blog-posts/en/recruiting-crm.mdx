...
The CRM I build. Open-source and self-hostable, with a managed application database configured in an EU region. From [[commercial.price.starter.monthly]]/user/month (Starter) or self-hosted for your own infrastructure cost. Configure the fixed contact, organization, deal, service, and task records with custom fields and relations for a recruiting workflow. The native unified inbox (LinkedIn, WhatsApp, email, Instagram, Telegram) brings connected-account conversations into the CRM from Pro, and authorized AI agents can draft or send outreach. Agent-native via MCP, full REST API, n8n community node, [[derived.webhooks.events.total]] webhook events. Best for: boutique agencies and in-house TA teams that want data ownership and AI tools driving the CRM directly.

**Concrete recruiting setup with Customermates.** Configure contacts as candidates, organizations as hiring clients, and deals as jobs or placements, then add the recruiting-specific custom fields and pipeline stages you need. Clean and deduplicate the existing spreadsheet before loading supported records through the REST API or an AI agent over MCP; there is no CSV importer. On Pro, connect [email](/features/email-integration) and use the imported read-only calendar view. If you need a placement workflow that creates a draft in lexoffice and alerts the back office, run it in your own n8n instance or another automation service through API and webhooks. Setup time depends on data quality, mappings, permissions, and the external workflow. See [self-hosted](/features/self-hosted) for the hosting model and the [pipeline feature](/features/pipeline) for the underlying mechanics.

**Concrete cost over 3 years (8 recruiters).** Cloud at [[commercial.price.starter.monthly]]/user × 8 = [[commercial.price.starter.monthly.seats.8]]/month → [[commercial.price.starter.monthly.seats.8.months.36]] over 3 years. Self-hosted on a Hetzner CX21 EU VPS at €5/month + admin time (~30 hours total) → €180 in server cost + maybe €1,500 in time at €50/hour = €1,680. Either way, multiples cheaper than Bullhorn ($14,000+) or Recruit CRM ($9,600+) at the same headcount. The trade-off is the smaller community and fewer pre-built recruiting-specific integrations.

### Pipedrive (configured)

Mainstream sales CRM, can be configured with custom pipeline stages and fields for recruiting. Limited candidate-specific features but mature platform. €39/user/month for the realistic tier (Growth). Best for: small agencies that already know Pipedrive and want a known quantity.
...
**Failure 1: Buying Bullhorn before you need it.** A 5-recruiter agency signs a $99/user contract because the demo was great. Year-one cost is $5,940 in license alone. The same agency would have run on Recruit CRM at $40/user (~$2,400) or [Customermates](/) at [[commercial.price.starter.monthly]]/user ([[commercial.price.starter.monthly.seats.5.months.12]] per year) and spent the difference on actual marketing.
...
[Customermates](/) exposes [[derived.mcp.tools.total]] MCP tools, REST API, and webhooks specifically for this. A recruiting agency can have Claude:
...
**Recruit CRM → Customermates self-hosted (8-recruiter boutique).** Migration: 4 weeks elapsed. CSV export was clean from Recruit CRM. Supported records were loaded through a customer-run [REST API](/features/api) integration that owned mapping, validation, and deduplication. Custom workflow rebuild: 30 hours. Internal cost: about $6,000. Annual savings: about $5,000 plus simpler administration. Payback: 1.2 years.
...
<Faq>
<FaqItem question="What is the difference between a recruiting CRM and an ATS?">
...
<FaqItem question="Can I use a sales CRM for recruiting?">
...
<FaqItem question="Is Bullhorn worth the cost?">
...
<FaqItem question="What recruiting CRM has the best AI features?">
...
<FaqItem question="How much does a recruiting CRM cost?">
