...
The CRM I build. Open-source, EU-hosted or self-hosted. From €12/user/month (€10 yearly, Starter) or self-hosted for server cost. Configurable: define candidate, job, submission, placement entities with custom fields. The native unified inbox (LinkedIn, WhatsApp, email, Instagram, Telegram) keeps candidate conversations across every channel in the CRM, and AI agents can draft and send outreach. Agent-native via MCP, full REST API, n8n community node, 26 webhook events. Best for: boutique agencies (1-15 recruiters), in-house TA teams that want data ownership, agencies that value AI tools (Claude, Codex) driving the CRM directly.
...
**Failure 1: Buying Bullhorn before you need it.** A 5-recruiter agency signs a $99/user contract because the demo was great. Year-one cost is $5,940 in license alone. The same agency would have run on Recruit CRM at $40/user (~$2,400) or [Customermates](/) at €12/user (~€720) and spent the difference on actual marketing.
...
[Customermates](/) exposes 46 MCP tools, REST API, and webhooks specifically for this. A recruiting agency can have Claude:
