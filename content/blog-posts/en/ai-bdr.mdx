...
[Customermates](/) is an open-source CRM built so any MCP-compatible AI agent can read and write the full CRM directly. There are [[derived.mcp.tools.total]] native MCP tools that cover contacts, organizations, deals, services, tasks, custom fields, widgets, and webhooks. An AI BDR connected over MCP does not "send leads to the CRM"; it operates the CRM.
...
3. `update_record_notes` with the summary and key signals from the thread
4. `create_tasks` with a follow-up for the AE on the day after the booked meeting
5. `manage_widgets` to reflect the new pipeline state on the team dashboard
...
**Customermates.** Not an AI BDR. The CRM that any of the above plug into via MCP, with [[derived.mcp.tools.total]] native tools. Open source under AGPL-3.0; cloud from [[commercial.price.starter.monthly]] per user per month (Starter) or free to self-host via Docker; the unified inbox that holds conversation history sits on Pro at [[commercial.price.pro.monthly]]. Best as the destination layer of the stack, where contacts, deals, and conversations actually live.
...
<FaqItem question="What is a BDR? What does BDR stand for?">
...
<FaqItem question="Will BDRs be replaced by AI?">
...
<FaqItem question="What does an AI BDR cost?">
...
<FaqItem question="Can an AI BDR really write personalized email?">
...
<FaqItem question="How does the AI BDR connect to my CRM?">
...
<FaqItem question="Can I use Claude or ChatGPT as my AI BDR?">
...
<FaqItem question="What is the smallest team that should consider an AI BDR?">
...
<FaqItem question="What replaces an AI BDR if it does not work?">
