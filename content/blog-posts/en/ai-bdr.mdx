...
[Customermates](/) is an open-source CRM built so any MCP-compatible AI agent can read and write the full CRM directly. There are 45 native MCP tools that cover contacts, organizations, deals, services, tasks, custom fields, widgets, and webhooks. An AI BDR connected over MCP does not "send leads to the CRM"; it operates the CRM.
...
**Customermates.** Not an AI BDR. The CRM that any of the above plug into via MCP, with 45 native tools. Open source under AGPL-3.0; cloud from €12 per user per month (Starter) or free to self-host via Docker; the unified inbox that holds conversation history sits on Pro at €29. Best as the destination layer of the stack, where contacts, deals, and conversations actually live.
