...
Use an n8n Webhook node or a CRM trigger. If you use [Customermates](/), new contacts automatically trigger workflows via the separate n8n integration. Alternatively, connect web forms, email parsers, or chatbot outputs.
...
**5. Update CRM.**
Add a CRM node to write the results. In Customermates, map the total score to a custom field, set the classification as a single-select custom column, and add the detailed scoring as a note. This makes the qualification visible on the deal card without opening a separate report.
...
| **Total (1 user)**            | **EUR 20 + EUR 5-10 + [[commercial.price.starter.monthly]]** |
...
| Cost                 | [[commercial.price.starter.monthly]]/user + EUR 5-10 API | From ~EUR 90/seat (Professional plan) | EUR 14/user + external tool  |
...
**Customermates.** From [[commercial.price.starter.monthly]]/user/month (Starter), the full core CRM included. Custom fields for scores, tags for classification, native n8n integration for automated updates, GDPR-compliant EU hosting. The native MCP integration ([[derived.mcp.tools.total]] tools) lets Claude, ChatGPT, or Gemini operate the CRM directly, and a unified inbox brings email, LinkedIn, WhatsApp, Instagram, and Telegram into one place on the Pro tier. Automation is included at every tier, so you do not pay extra for features that enterprise CRMs gate behind higher plans.

**HubSpot.** Lead scoring is available on the Professional plan (from about EUR 90 per seat per month). The built-in scoring is rule-based, not AI-powered. For AI scoring via n8n, the free CRM tier works but lacks advanced automation triggers.
...
<Faq>
<FaqItem question="Can ChatGPT really replace a human SDR for lead qualification?">
...
<FaqItem question="Which framework should I start with?">
...
<FaqItem question="How accurate is ChatGPT at scoring leads?">
...
<FaqItem question="Does the n8n workflow work with GPT-4o and GPT-3.5?">
...
<FaqItem question="How do I handle leads that ChatGPT scores incorrectly?">
...
<FaqItem question="Is it GDPR-compliant to send lead data to ChatGPT?">
When using the OpenAI API with a business agreement and DPA, data is processed but not used for training. For maximum compliance, minimize the PII sent (use company name and message content, not personal email addresses), and consider a self-hosted LLM via Ollama for sensitive data. On the CRM side, Customermates uses an EU region for the managed database and publishes a DPA and SCCs, but holds no compliance certification.
</FaqItem>

<FaqItem question="What if I only get 20 leads per month, is automation worth it?">
...
<FaqItem question="How do I handle leads from referrals that have little written data?">
