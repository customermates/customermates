...
**Customermates.** Open source, AGPL licensed, cloud from 12 euros per user per month or free to self host via Docker. The angle relevant to this article: the native unified inbox (on the Pro tier at 29 euros) puts email, LinkedIn, WhatsApp, Instagram, and Telegram in one place inside the CRM, and the AI agents you already use (Claude, ChatGPT, Gemini) can read, draft, and send across every channel through a native MCP integration. For rule-based outbound, an n8n workflow node plus 26 webhook events drive any message you need: deal stage changes trigger emails, custom field updates trigger SMS via any provider you connect. The communication layer lives inside the CRM instead of scattered across tools. See [pricing](/pricing) for the breakdown.
...
| Customermates | CRM with comms | EUR 29/user/month (Pro, unified inbox) | SMB sales and customer success, AI agent driven |
...
**One CRM as the source of truth.** Contacts, organizations, deals, custom fields, tasks. Customermates fits here from 12 euros per user per month with the open source escape hatch.
...
For small business, a CRM with built in communication features is usually a better fit than dedicated CCM software. Customermates from 12 euros per user per month (unified inbox on Pro at 29 euros), Pipedrive at 14 dollars per user per month, and HubSpot CRM (free tier) all cover sales and customer communication needs without the complexity or cost of enterprise CCM platforms like Quadient or OpenText.
...
Yes. Customermates is an open source CRM (AGPL 3.0) with built in communication features, n8n workflow integration, and 26 webhook events covering every record and messaging change. It is free to self host via Docker, with cloud tiers from 12 euros per user per month. For pure CCM in the document composition sense, open source options are limited because the category is dominated by enterprise vendors.
