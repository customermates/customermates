...
Cloud hosting in Germany from €9 per user per month yearly. Self-hosting via `docker compose up -d` is free under [AGPL-3.0](https://www.gnu.org/licenses/agpl-3.0.html). Migration from Excel: CSV export, CSV import into Customermates, configure custom fields. With an AI agent, the migration runs in one morning.
