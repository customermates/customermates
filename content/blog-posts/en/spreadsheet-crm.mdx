...
Yes. Customermates is licensed under AGPL-3.0 and free to self-host via `docker compose up -d`. Two containers—the app with its embedded workflow worker and PostgreSQL—are enough. Other options: EspoCRM, SuiteCRM, Krayin. The Customermates cloud version starts at €12 per user per month (Starter) and is hosted in Germany.
