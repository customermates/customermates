...
function sendOverdueFollowups() {
  ...
  for (const owner in overdueByOwner) {
    ...
    MailApp.sendEmail(
      `${owner.toLowerCase()}@yourcompany.com`,
      "Overdue follow-ups",
      body,
    );
  }
...
- **Open source, EU hosting, AI agent control**: Customermates from [[commercial.price.starter.monthly]] per user per month (Starter), free to self-host.

## From spreadsheet CRM to an open-source alternative

Customermates is built for exactly this transition. The Table view (one of two default views alongside the Card/Kanban view) feels familiar to spreadsheet users: columns, rows, filters, sorting. Ten typed custom field types (plain, single-select, currency, date, date range, dateTime, dateTime range, email, link, and phone) cover most columns you bring from Excel, now with validation and search indexing.

The decisive difference from spreadsheet world: Customermates can be operated by AI agents. Through a built-in [MCP server](https://modelcontextprotocol.io) with [[derived.mcp.tools.total]] tools, Claude, ChatGPT, or Codex can read and write inside the CRM directly, including a native unified inbox across email, LinkedIn, WhatsApp, Instagram, and Telegram. In practice: you forward an email to Claude, and Claude creates the contact, writes the note, updates the deal stage. The data hygiene that's manual in a spreadsheet runs through the agent instead.

Cloud hosting in the EU region from [[commercial.price.starter.monthly]] per user per month (Starter tier). Self-hosting via `docker compose up -d` is free under [AGPL-3.0](https://www.gnu.org/licenses/agpl-3.0.html). Migration from Excel: Export the sheet, create the records through the REST API or an AI agent over MCP, configure custom fields. With an AI agent, the migration runs in one morning.
...
<Faq>
<FaqItem question="Can I use Excel as a CRM?">
...
<FaqItem question="When should I switch from a spreadsheet to a CRM?">
...
<FaqItem question="What columns should a sales tracking spreadsheet have?">
...
<FaqItem question="Is Google Sheets better than Excel for a CRM?">
...
<FaqItem question="Is there a free open-source CRM I can use instead?">
Yes. Customermates is licensed under AGPL-3.0 and free to self-host via `docker compose up -d`. Three containers, 30 minutes, the system is ready. Other options: EspoCRM, SuiteCRM, Krayin. The Customermates cloud version starts at [[commercial.price.starter.monthly]] per user per month (Starter) and is hosted in the EU region.
</FaqItem>

<FaqItem question="How many users can collaborate on a Google Sheets CRM?">
...
<FaqItem question="What's the cheapest path from spreadsheet to a real CRM?">
Self-host an open-source CRM. Customermates runs on a €5/month VPS via `docker compose up -d`, free under AGPL-3.0, your only cost is the server. EspoCRM, SuiteCRM, and Krayin work the same way. If you don't want to manage a server, the cheapest cloud option among modern CRMs is HubSpot Free (genuinely free for up to 1,000 contacts) or Customermates Cloud from [[commercial.price.starter.monthly]] per user per month (Starter). Pipedrive starts at $14, Salesforce at $25.
</FaqItem>
</Faq>
