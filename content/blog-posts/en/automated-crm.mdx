...
**Layer 1: Native automation rules.** Field-change triggers, time-based reminders, simple multi-step sequences. Every modern CRM has this. Customermates ships rule-based [workflow automation](/features/workflow-automation) plus 26 webhook events that fire on key state changes.
...
Customermates checks all eight. The [pricing](/pricing) starts at €12/user/month (€10 annually, Starter tier); workflow automation, webhooks, and the MCP server are included at every tier, with no separate "automation tier."
...
**Native workflow automation.** Rule-based [workflow automation](/features/workflow-automation) with conditional logic, time delays, and 26 webhook events covering every state change. Free with the base plan, no separate tier.
...
**45 MCP tools.** Native MCP server exposes every CRM operation (create contact, update deal, log activity, query pipeline, generate report) to Claude, ChatGPT, Gemini, or any other MCP-compatible agent. The agent reads the inbox and writes back to the CRM in plain English.
...
**Customermates.** Open-source under AGPL-3.0, EU-hosted cloud from €12/user/month (Starter) or self-hosted free. Native [workflow automation](/features/workflow-automation), 26 webhook events, [n8n community node](/features/integrations), a native unified inbox (email, LinkedIn, WhatsApp, Instagram, Telegram on Pro and up), and 45 MCP tools for direct AI agent operation. Smaller ecosystem than HubSpot or Salesforce; the agent-native architecture is the differentiator. Best for technical SMBs or any team already running Claude, ChatGPT, or Gemini daily.
