...
| **Customermates** | 10 types, unlimited per entity | 5 built-in entities, all custom-fieldable | Yes, full + 26 webhooks + MCP | Yes (Docker) | From €12/user/mo |
...
If you want a customizable CRM that already includes the API, webhooks, custom fields, and an MCP server for AI agent automation, [Customermates](/pricing) ships from €12 per user per month. The first seven days on Pro are free, no credit card. If self-hosting is a hard requirement, the same product is on [GitHub](https://github.com/customermates/customermates) under AGPL-3.0.
