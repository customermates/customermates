...
Salesforce offers three pricing models for Agentforce. All add to your existing Salesforce bill, and Flex Credits and Conversations cannot be combined in the same org.
...
**Flex Credits.** Roughly $0.10 per agent action (each action consumes 20 credits, and 100,000-credit packs sell for $500). An action is roughly one tool call: read a record, update a field, send an email. Voice actions cost 30 credits. Useful for variable internal workloads where conversation count is hard to predict.

**Per-user licensing.** A seat-based option starting at $125 per user per month, rising to $550 per user per month for the Agentforce 1 Sales edition. Useful when usage is high and predictable enough that a flat per-seat rate beats consumption pricing.
...
**Important context:** Agentforce requires Sales Cloud, Service Cloud, or Data Cloud as the host platform. Sales Cloud runs from $25/user/month (Starter Suite) and $100 (Pro Suite) to $175 (Enterprise) and $350 (Unlimited), all billed annually above Starter. Most teams that deploy Agentforce sit on Enterprise. A 25-user team running Sales Cloud Enterprise plus Agentforce realistically spends $5,000-$15,000/month before implementation costs.
...
**Small to mid-sized teams without existing Salesforce.** The total cost of getting started (Sales Cloud + Agentforce + implementation partner) starts around $50,000/year for a small team. For comparison, Customermates uses the current CRM subscription plus the chosen model provider's metered API usage, so compare the stacks with your actual team size and usage rather than a fixed all-in estimate.
...
**Salesforce Agentforce.** Closed-source, hosted, integrated with Salesforce platform. Best for enterprise Salesforce shops with budget. Price: $2/conversation, ~$0.10/action, or from $125/user/month, plus the Salesforce license.
...
**Open-source CRM + external LLM via MCP.** [Customermates](/blog/open-source-crm) plus Claude, Codex, or any MCP-compatible model. Open-source under AGPL-3.0, self-hostable, EU-hosted cloud option. Price: from [[commercial.price.starter.monthly]]/user/month for the CRM (Starter; Pro at [[commercial.price.pro.monthly]] adds the unified inbox) plus your AI API costs (typically $5-50/month for small teams).
...
For many teams under 100 users, the open-source path delivers much of the relevant agent capability at materially lower software cost. For enterprise with existing Salesforce investment, Agentforce extends what you have rather than replacing it.

## Customermates as an open-source Agentforce alternative

Customermates exposes [[derived.mcp.tools.total]] MCP tools that let Claude, Codex, ChatGPT, or any other MCP-compatible agent operate the full CRM:
...
The CRM is open-source under AGPL-3.0. Self-hosted, the cost is the server rent (Hetzner CX21 at €5/month runs comfortably for a 20-person team). Cloud-hosted, [pricing](/pricing) starts at [[commercial.price.starter.monthly]]/user/month (Starter), with EU hosting (Germany) as the default.
...
**Underestimating the platform tax.** Buyers focus on the per-conversation Agentforce price ($2/conversation) and forget the underlying Sales Cloud or Service Cloud license is the real cost driver. A 25-user team running Sales Cloud Enterprise plus Agentforce realistically spends $5,000-$15,000/month before the first agent ships. The Agentforce line item is rarely the largest. Build the budget assuming the full Salesforce stack.
...
<Faq>
<FaqItem question="Is Agentforce just a chatbot?">
...
<FaqItem question="Does Agentforce work without Salesforce?">
...
<FaqItem question="How is Agentforce different from Einstein?">
...
<FaqItem question="Can I use Claude or GPT with Agentforce?">
...
<FaqItem question="What is the best Agentforce alternative for small teams?">
For teams under 50 users without existing Salesforce, the most cost-effective alternative is an open-source CRM with MCP support plus your preferred LLM. [Customermates](/blog/open-source-crm) plus Claude or ChatGPT delivers much of the relevant agent experience at materially lower software cost. The trade-off is a smaller integration ecosystem; for many small teams, that is not the binding constraint.
</FaqItem>

<FaqItem question="What does Agentforce cost for small teams?">
The minimum realistic cost for Agentforce is the Salesforce platform license plus the Agentforce usage. For a 10-user team, that is roughly $2,000-$5,000/month for Sales Cloud Enterprise plus Agentforce, before implementation. Most small teams cannot justify that. For comparison, the same team's Customermates Starter subscription is [[commercial.price.starter.monthly.seats.10]] per month before the chosen provider's metered Claude API usage.
</FaqItem>

<FaqItem question="How do I migrate off Agentforce later if needed?">
...
<FaqItem question="Is Agentforce GDPR compliant?">
...
<FaqItem question="Where does Agentforce go from here?">
