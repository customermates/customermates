...
**Transparent pricing.** From EUR 12 per user per month with the full core CRM included. No per-contact charges, no add-on costs to unlock the basics. That is barely more than many Google Workspace subscriptions.
...
The good news is that modern CRM software has become remarkably affordable and easy to adopt. Customermates, from EUR 12 per user per month, provides everything a Google Sheets CRM cannot: relational data, [workflow automation](/features/workflow-automation), pipeline visualization, email integration, GDPR compliance, and MCP integration for external AI clients, all in an [open-source](/blog/open-source-crm), self-hostable package.
