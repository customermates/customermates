...
The server provides connection instructions describing its tool surface, safety rules, and how to retrieve relevant Workspace Wiki pages. Clients decide whether to pass those instructions to their agent. If yours does not, add the Wiki retrieval rule to that client's persistent instructions. In clients that support MCP prompts (like Claude Code), you can run the built-in `get-started` prompt for a guided kickoff: it interviews you, then summarizes your workspace.
