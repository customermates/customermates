...
In Customermates, open **My Profile → API & Connectors**, press **Add** and choose **Standard API key**. Copy the 64-character string immediately. It is shown once. Creating keys needs the **API & Webhooks** permission in your role, see [API keys](/docs/api-keys).
</Step>
...
An **Authorize access** screen names the app that is connecting and the destination it will send access to.
</Step>
<Step title="Approve">
Click **Approve**.
</Step>
...
- Every call runs as the Customermates user who approved, inside that account's data and with that user's role permissions. Connecting over OAuth needs no API key and no extra role permission. API keys inherit the owning user's permissions; there is no per-key scoping.
