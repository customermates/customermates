...
Starting setup opens a visible conversation titled **Set up Workspace Wiki**. The Wiki step shows that setup is working and restores that state after a refresh; choose **Continue** when you are ready to move on, or reopen the Mate conversation from the step. Mate reads the submitted page directly and may read up to four useful pages linked from that homepage on the same domain or its subdomains.

When the site contains usable company information, Mate creates these five localized pages in one atomic empty-Wiki-only call: **Company Overview**, **Products & Services**, **Customers, Positioning & Competitors**, **Voice & Tone**, and **Support & FAQ**. The server supplies their localized titles and stable links between them. Mate adds ordinary source links and a clearly marked gaps section instead of inventing missing facts. Setup uses your normal hosted AI credits.
...
Read a result from `search`, including full Workspace Wiki Markdown by `wiki:<uuid>` or its exact returned URL. Internal Wiki links are absolute and directly followable. Wiki Read is required for Wiki content.
