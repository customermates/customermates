...
| `get_records` | ✓ | | Full record data by id, up to 100, mixed entity types allowed, optionally with master data or notes. |
| `create_contacts` | | | Create up to 100 contacts, custom-column values and relation ids inline. |
| `create_organizations` | | | Create up to 100 organizations. |
| `create_deals` | | | Create up to 100 deals, services as an inline array. |
| `create_services` | | | Create up to 100 services. |
| `create_tasks` | | | Create up to 100 tasks. |
| `update_contacts` | | | Partial update by contact key (id, email, phone, or provider:value). Never touches relations. |
| `update_organizations` | | | Partial update by id. Never touches relations. |
| `update_deals` | | | Partial update by id, including singleSelect custom-column values. Never touches relations. |
...
| `delete_records` | | ✓ | IRREVERSIBLE delete of 1 to 100 records by id. |
...
| Tool | Read | Destructive | Purpose |
|---|---|---|---|
| `get_workspace_context` | ✓ | | Your user, the company profile, all roles including permissions, and every connected messaging account in one call. The natural first call of a session. |
...
| Tool | Read | Destructive | Purpose |
|---|---|---|---|
| `get_messaging_threads` | ✓ | | Two modes: without threadId it lists inbox threads with filters and sorting (threads without any message yet are hidden unless they hold a draft), with threadId it returns one thread with the full message list including drafts. |
| `get_activities` | ✓ | | Activity timeline (messages, audit-log changes, calendar events) for the workspace or one record. |
| `send_chat_message` | | | Delivers immediately. With threadId it replies in an existing chat, with connectedAccountId plus attendeeIdentifiers it starts a new one. New LinkedIn chats can be sent as Sales Navigator or Recruiter InMail via linkedinProduct plus inmailSubject. |
...
| Tool | Read | Destructive | Purpose |
|---|---|---|---|
| `get_social_posts` | ✓ | | Posts of a connected LinkedIn or Instagram account. With postId it fetches a single post; authorIdentifier defaults to the account owner. |
...
| `manage_social_relations` | | | Connection requests: list received invitations, invite (sends a real request), accept, or cancel by invitationId. |
| `search_sales_leads` | ✓ | | Finds people via Sales Navigator: either from a pasted search URL or as a structured search with filters (keywords, location, industry, company, job title, seniority and more). Requires a Sales Navigator subscription. |
| `search_sales_companies` | ✓ | | Finds companies via Sales Navigator: either from a pasted company search URL or as a structured search with filters (keywords, location, industry, headcount, annual revenue and more). Requires a Sales Navigator subscription. |
| `get_sales_search_parameters` | ✓ | | Resolves the parameter ids used by the Sales Navigator search filters (locations, industries, job titles, companies, lists and more) by type and keyword. Also finds existing lead and account lists by name. |
| `manage_sales_lists` | | | Sales Navigator lead and account lists: list them, browse the members of one, or save a lead or company to an existing list. New lists are created in Sales Navigator itself. |

### Documentation and deep research

| Tool | Read | Destructive | Purpose |
|---|---|---|---|
| `search_docs` | ✓ | | Full-text search over the product guides and REST reference. Returns slug, title, url, snippet. |
