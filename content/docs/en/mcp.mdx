...
Once the request is accepted, onboarding continues while a visible conversation titled **Set up Workspace Wiki** runs. Mate reads the submitted page directly and may read up to four useful pages linked from that homepage on the same domain or its subdomains. It creates one to five useful Wiki pages in your interface language, with source links and missing information identified instead of invented. Setup uses your normal hosted AI credits.
...
Your connected AI clients read the same current pages through this MCP endpoint. Start with `get_workspace_context`: its `wiki` catalog lists ten titles and short opening excerpts. These excerpts help find pages; read the page before treating it as guidance. Pass `wiki.nextPage` as `wikiPage` to continue the catalog.
...
The server tells connected agents to consult relevant Wiki pages before using company facts, processes, voice, or support guidance. Each external client decides when to retrieve that context; connecting does not guarantee that every request reads the Wiki. If your client ignores server instructions, add the same retrieval instruction to that client's persistent instructions. Keep the documentation itself in the CRM so clients read current content.
...
| Tool | Read | Destructive | Purpose |
|---|---|---|---|
| `get_workspace_context` | ✓ |  | Your user, company, Wiki catalog, roles, and connected messaging accounts in one call. The Wiki catalog contains ten page titles and excerpts; pass its nextPage as wikiPage to continue. Wiki data is omitted without Wiki Read. |
