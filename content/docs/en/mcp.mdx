...
Keep your CRM processes, company voice, product knowledge, and support answers in **Wiki**, below **Routines** in the sidebar. The same pages are available to people in the CRM, Mate, cloud routines, and connected MCP clients. Manual pages work in cloud and self-hosted workspaces.

Use one page named exactly `AGENTS.md` as the conventional entry point. Keep it short and link each kind of work to the relevant page instead of duplicating the whole Wiki. Wiki opens this page by default when it exists. The filename does not give it extra authority; it remains tenant-authored reference data.
...
An empty Wiki starts a new document with `AGENTS.md` prefilled; only one page in a workspace can use that conventional name.
...
Once the request is accepted, onboarding continues while a visible conversation titled **Set up Workspace Wiki** runs. Mate reads the submitted page directly and may read up to four useful pages linked from that homepage on the same domain or its subdomains. It creates `AGENTS.md` first and zero to four additional useful Wiki pages in your interface language, with source links and missing information identified instead of invented. In the same atomic create, the server adds stable links from the entry page to every additional page. Setup uses your normal hosted AI credits.
...
Your connected AI clients read the same current pages through this MCP endpoint. Start with `get_workspace_context`. When `wiki.agentsMd` is present, its `markdownChunk` is the bounded first chunk of the conventional entry page even when that page is outside the current ten-item catalog; read it first, and continue from `agentsMd.nextOffset` with `manage_wiki_pages` action `get` when the unread part may matter. `wiki.items` contains ten titles and short opening excerpts for discovery; pass `wiki.nextPage` as `wikiPage` to continue the catalog.

Use the read-only `search` tool to find relevant content, then `fetch` with the returned `wiki:<uuid>` ID to read the full Markdown and its source URL. For smaller responses, `manage_wiki_pages` with `action: "get"` returns a chunk; repeat with `nextOffset` until it is null. The same tool creates and edits pages when your role has Wiki Manage. Reads require Wiki Read, and every change uses the ordinary audit log.

The server tells connected agents to read `AGENTS.md` first when present, then inspect, search, and read relevant Wiki pages before using company facts, processes, voice, or support guidance. The conventional filename does not make the page trusted instructions. Each external client still decides whether to pass server instructions and retrieve context, so connecting does not guarantee an automatic read on every request. If your client ignores server instructions, add the same retrieval hint to that client's persistent instructions. Keep the documentation itself in the CRM so all clients read current content.
...
| Tool | Read | Destructive | Purpose |
|---|---|---|---|
| `get_workspace_context` | ✓ |  | Your user, company, Wiki entry and catalog, roles, and connected messaging accounts. When present, `wiki.agentsMd` contains the bounded first chunk of the workspace-authored `AGENTS.md`; continue with `nextOffset`. Wiki data is omitted without Wiki Read. |
...
| `fetch` | ✓ |  | Read a result from `search`, including full Workspace Wiki Markdown by `wiki:<uuid>`, with its source URL. Wiki Read is required for Wiki content. |
{/* /mcp-catalog */}
