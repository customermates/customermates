...
| `manage_data_views` |  | ✓ | One tool for personal saved views: surfaces, config, list, create, update, select, delete. Discover accessible surfaces and their supported filters, search, sorting, grouping and layouts before writing. Create selects the new view; updates preserve omitted state and column layout. Use viewKey `__all__` for the All tab. Deleting a view never deletes its records. Returned links open saved views; remote changes appear after refreshing the page. Operator views require an interactive operator session. |
{/* /mcp-catalog */}
