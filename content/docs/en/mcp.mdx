...
Required: `entity`. Optional: `searchTerm`, `filters`, `sortDescriptor`, `page`, `pageSize`.

#### `search_records`

Free-text search across one or more entity types in one call.

Required: `searchTerm`. Optional: `entities`, `limitPerEntity`.

#### `get_records`

Full record data by id, up to 100, mixed entity types allowed; contacts also by email, phone, or provider:handle. Always returns fields; add markdown notes per item with include=withNotes.
...
Required: `messageId`, `draftRevision`.
...
Required: `connectedAccountId`. Optional: `postId`, `authorIdentifier`, `cursor`, `offset`, `limit`.
...
Required: `connectedAccountId`, `postId`. Optional: `kind`, `commentId`, `sortBy`, `cursor`, `offset`, `limit`.
...
Required: `connectedAccountId`, `identifier`. Optional: `profileType`.
...
Required: `connectedAccountId`. Optional: `url`, `filters`, `offset`, `limit`.
...
Required: `connectedAccountId`, `type`. Optional: `keywords`, `offset`, `limit`.
...
| Tool | Read | Destructive | Purpose |
|---|---|---|---|
| `manage_custom_columns` |  | ✓ | One tool with an `action` switch: list, upsert (create or update), delete. Upsert requires `intent` (`create` or `update`). Covers all ten column types; label, type and entityType are immutable on update. Delete is IRREVERSIBLE and removes every stored value. |
...
Required: `action`. Optional: `entityType`, `id`, `intent`, `type`, `label`, `selectOptions`, `options`.
{/* /mcp-catalog */}
...
Required: `action`. Optional: `kind`, `id`, `ids`, `name`, `entityType`, `entityFilters`, `dealFilters`, `displayType`, `groupByType`, `groupByCustomColumnId`, `aggregationType`, `reverseXAxis`, `reverseYAxis`, `barColors`, `timelineFilters`, `showFilters`.
{/* /mcp-catalog */}
...
Required: `action`. Optional: `id`, `cursor`, `page`, `pageSize`, `searchTerm`, `name`, `prompt`, `enabled`, `triggerKind`, `cronExpression`, `timezone`, `triggerEvents`, `changedFields`, `triggerFilters`, `debounceSeconds`.
{/* /mcp-catalog */}
...
Required: `action`. Optional: `id`, `url`, `description`, `events`, `secret`, `headers`, `bodyTemplate`, `enabled`, `searchTerm`, `filters`, `sortDescriptor`, `page`, `pageSize`.
{/* /mcp-catalog */}
...
Open a support ticket with the Customermates team (subject plus description). The team follows up by email and in the in-app chat. Returns the ticket number.
