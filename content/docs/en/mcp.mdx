...
In the app, open **Profile**, then **API & Connectors**, and add a key. Choose the guided setup for Claude, ChatGPT, Cursor, or Gemini, or create a standard key for your own integration. The key is shown once, so copy it before closing the dialog.
</Step>
...
Send your 64-character API key in the `x-api-key` header, or connect through OAuth where the client supports it. Keys are created in the app under Profile, in API & Connectors, and every key is scoped to your user and workspace permissions.
</FaqItem>
<FaqItem question="Which AI clients can connect?">
Claude on web, mobile and desktop, ChatGPT, Claude Code, Codex and Cursor are documented above with a guide per client. Anything that speaks MCP over streamable HTTP works with the same endpoint.
</FaqItem>
...
Every destructive tool is flagged in the catalog above and starts its description with `IRREVERSIBLE`. Clients that honor MCP annotations also receive `destructiveHint` and can ask for confirmation before calling.
</FaqItem>
