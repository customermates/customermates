...
| Tool | Read | Destructive | Purpose |
|---|---|---|---|
| `get_social_posts` | ✓ | | Posts on LinkedIn or Instagram, read through a connected account. Lists a person's posts via authorIdentifier (`me` or a top-level profile/participant `id`; never public_identifier or member_id), or fetches one post by postId. Omit cursor and offset initially, then reuse next_cursor; LinkedIn user posts use cursor pagination. |
...
| `get_social_profile` | ✓ | | A person or company profile. For a person, use `profileType=person` with `me`, a top-level profile/participant `id`, a LinkedIn Classic public_identifier, or an Instagram username; never use member_id. For a LinkedIn company, use `profileType=company` with its company id. |
