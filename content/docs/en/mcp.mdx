...
- **Relationship safety**: relations change only through `manage_record_links` (add, remove, or set to replace the whole list in one call, keeping links to records outside the caller's access); passing `null` on a relationship array is rejected with a remediation hint.
...
All create and update tools take custom-column values via customFieldValues; call `get_record_schema` first for the column ids. `list_records` returns `total` and, for entities with numeric columns, `sums` before the items; for deals `sums` carries `totalValue` and `weightedValue`, the pipeline weighted by each stage's win probability, across every record matching the filters. With `include`, each listed item also carries the fields asked for: `owners` adds `userIds`, `links` the ids of the linked records such as `dealIds` and `taskIds`, `customFields` the `customFieldValues` with single-select values as option ids, and `dates` `createdAt` and `updatedAt`. A relation your role cannot read is left out, so a missing key means you cannot read that relation and an empty array means none you can see is linked. `pageSize` accepts any whole number from 1 to 100 and returns exactly that many records per page, fewer only on the last; `page` counts in that size, so page 2 at `pageSize` 50 holds records 51 to 100, and the result reports both. With `groupBy` (a single-select column, a relation such as `userIds`, or `createdAt` or `updatedAt` by day, week or month), `list_records` returns `groups` with each group's label and count, and for deals the `totalValue` and `weightedValue` sums, instead of items. Grouping by `createdAt` or `updatedAt` covers the last 7 days, the last 7 weeks or the last 12 months, month being the default, each up to and including the current one: older records are counted together in one `before` group, records dated after the current period in one `from` group, and `groupNote` points this out whenever either of them holds records.
...
Search, filter, sort, paginate one entity type, or count and total it per group with groupBy. Always returns the total. Deals include totalValue and totalQuantity, services include amount.

**Read-only.** Required: `entity`. Optional: `searchTerm`, `filters`, `sortDescriptor`, `page`, `pageSize`, `groupBy`, `include`.
...
Add, remove or replace the ids on one relation (`action` add, remove or set); set makes the ids the complete list in one call and keeps links to records outside your access. The only way to change relations.
