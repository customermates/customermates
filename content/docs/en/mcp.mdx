...
When Mate is enabled in the cloud workspace, the owner can optionally enter a company homepage during onboarding or from an empty Wiki and choose **Build my Wiki**. A bare domain such as `customermates.com` is accepted. **Not now** continues onboarding without creating pages; invited members do not receive this step.

Starting setup opens a visible conversation titled **Set up Workspace Wiki**. The Wiki step shows that setup is working and restores that state after a refresh; choose **Continue** when you are ready to move on, or reopen the Mate conversation from the step. Mate reads the submitted page directly, then attempts up to three useful pages linked from that homepage on the same domain or its subdomains.

When the site contains usable company information, Mate creates these five localized pages in one atomic empty-Wiki-only call: **Company Overview**, **Products, Services & Value**, **Customers, Market & Competition**, **Voice, Tone & Messaging**, and **Sales, Onboarding & Support**. Together they cover public company context, offerings, audiences, positioning, communication style, and the customer lifecycle. The server supplies their localized titles, source links from pages Mate read, and stable links between them. Mate clearly marks missing internal decisions instead of inventing them. Setup uses your normal hosted AI credits.

Automatic setup focuses on durable company, product, and support knowledge. It omits volatile pricing, plan names, quotas, and conditional allowances so you can add the current commercial guidance after review. Review generated facts and their sources before relying on them; you can edit the pages normally.

Setup creates pages only while the Wiki is empty and never overwrites existing pages. If the site cannot be read or provides no useful company information, the conversation explains the problem and creates nothing. The Wiki step preserves the submitted address so you can retry or create a page manually. Once created, the five pages are ordinary editable Wiki documents.
...
Use the standard read-only `search` tool to find relevant content, then `fetch` with either the returned `wiki:<uuid>` ID or its exact URL. Wiki Markdown is returned in bounded chunks with a canonical source URL; repeat `fetch` with `nextOffset` as `offset` until it is null. Links between Wiki pages are absolute and can be passed directly back to `fetch`. Clients with MCP resource support can also discover the paginated catalog and page templates with `resources/list` and `resources/templates/list`. `manage_wiki_pages` with `action: "get"` uses the same continuation pattern and also creates or edits pages when your role has Wiki Manage. Reads require Wiki Read, and every change uses the ordinary audit log.
...
After this feature is deployed, an Enterprise or Edu workspace admin must **Refresh** the existing ChatGPT connector and review its new actions so ChatGPT discovers `manage_wiki_pages` and the current server instructions. Business workspaces must recreate and republish the connector when its approved tools or metadata change. Existing `search` and `fetch` calls use the updated Wiki-aware behavior automatically on the server.
...
**IRREVERSIBLE:** `delete_records`, `manage_wiki_pages`, `discard_message_draft`, `manage_custom_columns`, `manage_widgets`, `manage_routines`, `manage_webhooks`
{/* /mcp-catalog */}
...
Your user, company, permission-filtered Wiki catalog, roles, and connected messaging accounts. Pass `wikiPage` for catalog pagination and `wikiQuery` for up to three bounded matches from the whole Wiki. Wiki data is omitted without Wiki Read.

**Read-only.** Optional: `wikiPage`, `wikiQuery`.
...
List, search, read, create, update, or permanently delete Workspace Wiki pages. Updates and deletes require the latest updatedAt value; create can atomically require an empty Wiki.

**IRREVERSIBLE.** Required: `action`. Optional: `id`, `query`, `offset`, `page`, `pageSize`, `pages`, `requireEmpty`, `expectedUpdatedAt`, `title`, `markdown`.
{/* /mcp-catalog */}
...
Read-only search across Workspace Wiki content, CRM records, and product docs. Wiki results have title-weighted query matching and stable `wiki:<uuid>` ids for `fetch`. Compatible with ChatGPT company knowledge and deep research.

**Read-only.** Required: `query`.

#### `fetch`

Read a result from `search`, including bounded Workspace Wiki Markdown chunks by `wiki:<uuid>` or its exact returned URL. Continue with `nextOffset` until it is null. Internal Wiki links are absolute and directly followable. Wiki Read is required for Wiki content.

**Read-only.** Required: `id`. Optional: `offset`.
{/* /mcp-catalog */}
