...
- **Explicit column intent**: new integrations create with `intent=create` and no id, while legacy id-less calls remain create-only. Updates require `intent=update` plus an existing id. For `singleSelect`, pass the full choice list in top-level `selectOptions`; legacy `options.options` remains accepted, but never pass both. The tool keeps `label`, `type`, and `entityType` fixed on update, so a different label or type requires a new column.
...
| Tool                    | Read | Destructive | Purpose                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| ----------------------- | ---- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `manage_custom_columns` |      | ✓           | One tool with an `action` switch: list, upsert (create or update), delete. New integrations use explicit `intent=create` without an id; legacy id-less creates remain accepted. Updates require `intent=update` with an existing id. For `singleSelect`, prefer top-level `selectOptions`; legacy `options.options` is still accepted. Covers all ten column types; label, type and entityType are immutable on update. Delete is IRREVERSIBLE and removes every stored value. |
...
| Tool                        | Read | Destructive | Purpose                                                                                                                                                                |
| --------------------------- | ---- | ----------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `update_workspace_settings` |      |             | `target` profile updates your own name, country, and avatar; `target` company updates the workspace currency and the preset names used for core entities (admin only). |
...
| Tool              | Read | Destructive | Purpose                                                                                                                                                                                                                                                  |
| ----------------- | ---- | ----------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `request_support` |      |             | Email a support request to the Customermates team (subject plus description). Include all relevant context in the description. The team replies to the email address on your account. Returns confirmation after the email provider accepts the request. |
