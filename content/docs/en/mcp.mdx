...
Keep your CRM processes, company voice, product knowledge, and support answers in **Wiki**, below **Routines** in the sidebar. The same permission-checked pages are available to people in the CRM, Mate, cloud routines, and connected MCP clients. Manual pages work in cloud and self-hosted workspaces. There is no required index filename or privileged page: discovery is generated from the current Wiki for each request.

### Write and link pages

Choose **New page**, enter a title, and write directly in the visual editor. It uses the same formatting as CRM Notes. Choose **Save** when you are ready; changes are not saved automatically. **Reset** discards unsaved edits. Use the page list to switch documents or search their titles and content.

An empty Wiki starts a blank document. Give each page a clear, specific title so people and agents can find it. A title such as `AGENTS.md` has no special behavior and duplicates are allowed.

Use the editor's link control to find another Wiki page, or choose **Copy page link** in **Page actions**. These are ordinary Markdown links; renaming a page does not change its address. You do not need to maintain a separate index or linking metadata.

Wiki Read lets you view pages. Wiki Manage lets you create, edit, and delete them and includes Read. If another person or agent changes a page before you save, your stale save is refused and your edits stay visible so you can reconcile them with the latest page. Deletion is permanent. Changes use the normal Audit Logs; viewing their Wiki content requires both Audit Log access and Wiki Read. The audit log is not a restore feature.

### Set up from your homepage

When Mate is enabled in the cloud workspace, the owner can optionally enter a company homepage during onboarding or from an empty Wiki and choose **Set up with Mate**. A bare domain such as `customermates.com` is accepted. **Skip** continues onboarding without creating pages; invited members do not receive this step.

Once the request is accepted, onboarding continues while a visible conversation titled **Set up Workspace Wiki** runs. Mate reads the submitted page directly and may read up to four useful pages linked from that homepage on the same domain or its subdomains. It creates one to five useful ordinary Wiki pages in your interface language, with natural titles, source links, and missing information identified instead of invented. The pages are created in one atomic empty-Wiki-only call. Setup uses your normal hosted AI credits.

Automatic setup focuses on durable company, product, and support knowledge. It is instructed to link to current pricing rather than copy prices and conditional plan allowances into the Wiki. Review generated facts and their sources before relying on them; you can edit the pages normally.

Setup creates pages only while the Wiki is empty and never overwrites existing pages. If the site cannot be read or provides no useful company information, the conversation explains the problem and creates nothing. You can retry from the empty Wiki or create pages manually. Once created, the pages are ordinary editable Wiki documents.

This setup reads known public pages, not search-engine results. General web search is currently unavailable in Mate and routines. The public MCP endpoint exposes Wiki retrieval and editing, not Mate's public-page reader.

### Read the Wiki from an AI client

Your connected AI clients read the same current pages through this MCP endpoint. Start with `get_workspace_context`; `wiki.items` contains ten titles, stable URLs, and short opening excerpts, and `wiki.nextPage` can be passed as `wikiPage` to continue. Pass the current request as `wikiQuery` to also receive bounded previews of up to three matching pages from anywhere in the Wiki.

Use the standard read-only `search` tool to find relevant content, then `fetch` with either the returned `wiki:<uuid>` ID or its exact URL to read the full Markdown and canonical source URL. Links between Wiki pages are returned as absolute URLs and can be passed directly back to `fetch`. Clients with MCP resource support can also discover the paginated catalog and page templates with `resources/list` and `resources/templates/list`. For smaller responses, `manage_wiki_pages` with `action: "get"` returns a chunk; repeat with `nextOffset` until it is null. The same tool creates and edits pages when your role has Wiki Manage. Reads require Wiki Read, and every change uses the ordinary audit log.

The server tells connected agents to search and read relevant Wiki pages before using company facts, processes, voice, product, or support guidance, follow useful links, and cite the exact returned URLs. MCP clients decide whether they expose resources and server instructions to their model, so no server can force every arbitrary client to perform those reads. If your client ignores that guidance, add the same retrieval instruction to the client's persistent instructions. Keep the documentation itself in the CRM so every supported client retrieves the current authoritative copy.
...
| Tool | Read | Destructive | Purpose |
|---|---|---|---|
| `get_workspace_context` | ✓ |  | Your user, company, permission-filtered Wiki catalog, roles, and connected messaging accounts. Pass `wikiPage` for catalog pagination and `wikiQuery` for up to three bounded matches from the whole Wiki. Wiki data is omitted without Wiki Read. |
...
| Tool | Read | Destructive | Purpose |
|---|---|---|---|
| `manage_wiki_pages` |  | ✓ | List, search, read, create, update, or permanently delete Workspace Wiki pages. Updates and deletes require the latest updatedAt value; create can atomically require an empty Wiki. |
{/* /mcp-catalog */}
...
| `send_email` |  |  | Delivers immediately. Send or reply from a connected email account; sending a saved draft requires both draftMessageId and draftRevision. The account's enabled signature is appended automatically, so never write a sign-off into the body. |
| `save_message_draft` |  |  | Prepare a message for review: the draft shows up in the inbox and the user sends it. With `threadId` it drafts a reply; with `connectedAccountId` plus `recipients` it prepares a brand-new conversation that exists only as a draft. Returns the message id and opaque revision token; saving again updates the thread's one draft. The signature is appended when the draft is sent, so never write a sign-off into the body. |
| `discard_message_draft` |  | ✓ | Delete the exact saved draft revision using its message id and opaque revision token. |
| `update_messaging_thread` |  |  | Set the thread state: unread, open, closed, or spam. |
| `move_email_thread` |  |  | Move an email conversation into another mailbox folder at the provider. |
...
| `get_social_profile` | ✓ |  | A person or company profile. For a person, use `profileType=person` with `me`, `get_messaging_threads.items[].participants[].identifier`, `get_messaging_threads.thread.participants[].identifier`, `get_social_posts.items[].author.id`, `get_social_posts.author.id`, `get_social_post_engagement.items[].author.id`, `get_social_post_engagement.items[].sender.id`, `manage_social_relations.items[].user.id`, a LinkedIn Classic public profile slug, or an Instagram username. For a LinkedIn company, use `profileType=company` with `linkedin_search_sales_companies.items[].id`, `linkedin_search_sales_leads.items[].current_positions[].company_id`, `linkedin_manage_sales_lists.items[].current_positions[].company_id`, or `get_social_profile.current_positions[].company_id`. Reuse `get_social_profile.id` with the same `profileType`. |
| `manage_social_relations` |  |  | Connection requests: list invitations (received by default, or your own sent/outgoing via direction), invite with `get_social_profile.id` (sends a real request), accept, or cancel by invitationId. |
| `linkedin_search_sales_leads` | ✓ |  | Finds people via LinkedIn Sales Navigator: either from a pasted search URL or as a structured search with filters (keywords, location, industry, company, job title, seniority and more). Resolve `linkedin_search_sales_leads.items[].current_positions[].company_id` with `get_social_profile` and `profileType=company`. Requires a Sales Navigator subscription. |
| `linkedin_search_sales_companies` | ✓ |  | Finds companies via LinkedIn Sales Navigator: either from a pasted company search URL or as a structured search with filters (keywords, location, industry, headcount, annual revenue and more). Pass `linkedin_search_sales_companies.items[].id` to `get_social_profile` with `profileType=company`. Requires a Sales Navigator subscription. |
...
| `search` | ✓ |  | Read-only search across Workspace Wiki content, CRM records, and product docs. Wiki results have title-weighted query matching and stable `wiki:<uuid>` ids for `fetch`. Compatible with ChatGPT company knowledge and deep research. |
| `fetch` | ✓ |  | Read a result from `search`, including full Workspace Wiki Markdown by `wiki:<uuid>` or its exact returned URL. Internal Wiki links are absolute and directly followable. Wiki Read is required for Wiki content. |
{/* /mcp-catalog */}
...
| Tool | Read | Destructive | Purpose |
|---|---|---|---|
| `request_support` |  |  | Open a support ticket with the Customermates team (subject plus description). The team follows up by email and in the in-app chat. Returns the ticket number. |
{/* /mcp-catalog */}
...
Keys: records, workspace, wiki, messaging, social, docs, custom-columns, widgets, routines, webhooks, admin, support. No parameter means everything; unknown keys are ignored. `search` and `fetch` are always on so deep-research connectors keep working on any narrowed surface.
