...
| Tool | Read | Destructive | Purpose |
|---|---|---|---|
| `get_social_posts` | ✓ |  | Posts on LinkedIn or Instagram, read through a connected account. Use `authorIdentifier=me` for the account owner, or a top-level person `id` returned by a profile, post, comment, reaction, or relation-request result. Resolve a messaging participant's `identifier` through `get_social_profile` first. On continuation, repeat the same account, author, and limit with next_cursor. |
...
| `get_social_profile` | ✓ |  | A person or company profile. For a person, use `profileType=person` with `me`, `get_messaging_threads.participants[].identifier`, a returned social-user `id`, a LinkedIn Classic public profile slug, or an Instagram username. For a LinkedIn company, use `profileType=company` with `linkedin_search_sales_companies.id` or `current_positions[].company_id`. Reuse the returned `id` with the same `profileType`. |
...
| `linkedin_search_sales_leads` | ✓ |  | Finds people via LinkedIn Sales Navigator: either from a pasted search URL or as a structured search with filters (keywords, location, industry, company, job title, seniority and more). Lead rows include current positions; resolve a company id with `get_social_profile` and `profileType=company`. Requires a Sales Navigator subscription. |
| `linkedin_search_sales_companies` | ✓ |  | Finds companies via LinkedIn Sales Navigator: either from a pasted company search URL or as a structured search with filters (keywords, location, industry, headcount, annual revenue and more). Result `id` values can be passed to `get_social_profile` with `profileType=company`. Requires a Sales Navigator subscription. |
...
| `linkedin_manage_sales_lists` |  |  | LinkedIn Sales Navigator lead and account lists: list them, browse the members of one, or save a lead using `linkedin_search_sales_leads.id` or `get_social_profile.id`, or a company using `linkedin_search_sales_companies.id` or `current_positions[].company_id`. New lists are created in Sales Navigator itself. |

#### Typical social read flow

Choose a LinkedIn or Instagram entry whose `status` is `ok` from `get_workspace_context.connectedAccounts`, and use its `id` as `connectedAccountId`. When the person comes from an inbox thread, resolve `get_messaging_threads.participants[].identifier` first:
...
Call `get_social_profile` with that request, then pass its returned top-level `id` (`get_social_profile.id`) to `get_social_posts.authorIdentifier` for the first page:
...
For a company, call `get_social_profile` with `profileType=company` and an `identifier` from `linkedin_search_sales_companies.id` or `current_positions[].company_id`.
