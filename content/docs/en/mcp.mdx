...
The server sends workflow guidance when a client connects: read the schema first, find ids before writing, change relations only through `manage_record_links`, and name the exact records or recipients when deleting or sending. That guidance is not a server-enforced confirmation gate. Verify how the chosen client passes instructions to the model and handles approval before granting write, delete or messaging access. In clients that support MCP prompts, a built-in get-started prompt can summarize the workspace to personalize the start.
...
- **Client confirmation guidance.** The server instructions tell the model to name the exact records or recipients in the same message as a `delete_records` or send call, not to ask for permission and wait. The MCP route executes an authorized tool call once the client makes it, so any confirmation step is the client's, and verifying it is part of choosing one.
