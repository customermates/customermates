...
If you work with CLI-based MCP clients, tokens can be stored and reused there instead of being passed manually every time. A practical example is `mcporter`, which can list servers, call tools, and keep authentication configured in the client environment. The underlying API keys are managed in [Account Settings](/docs/account-settings), which is the right place to create or rotate credentials before connecting a client.
...
Create an API key in [Account Settings](/docs/account-settings), then configure your MCP client to connect to the Customermates MCP endpoint and send the key in the `x-api-key` header. If your client supports environment variable expansion, use that pattern for the header value instead of checking a raw key into config.
...
> For most agent setups, start with MCP when the model needs to reason about which action to take, and use intent detection to narrow the available tools whenever your runtime supports it. Then fall back to [OpenAPI 3.1.0](/docs/openapi) whenever you need exact payload structure, endpoint semantics, or implementation-level validation. For event-driven follow-up flows, pair this with [Webhooks & Events](/docs/feature-guide-webhooks-events).
