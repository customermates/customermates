...
A routine is a saved instruction plus a trigger. When the trigger fires, Customermates opens a new assistant conversation, sends your instruction, and lets the assistant work on its own. Everyone in the company can see the routine and its run summaries; the person who ran it can also open the full assistant transcript.
