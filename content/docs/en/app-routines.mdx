...
When the owner has Wiki Read, each run starts with a compact catalog of current [Workspace Wiki](/docs/mcp#shared-workspace-wiki) pages and can read the relevant documents. Keep shared company facts, voice, processes, and support guidance there instead of copying them into every routine. Creating or updating a Wiki page requires the owner's Wiki Manage permission.
