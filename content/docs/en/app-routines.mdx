...
When an event starts a run, the assistant is told what happened before your instructions: the
event name, the kind of record and its id, which fields changed on an update, and for an inbox
event the thread the message belongs to. It can read the record itself from there. It is not
given the previous values of the fields that changed.
...
An unattended run can read your workspace and create or update contacts, organizations, deals, services, tasks, notes, and Wiki pages within the owner's permissions. It can also draft and send messages.
...
Deleting records or Wiki pages always needs a person to approve it, so a routine that tries to delete something is declined immediately and the run records that it was refused.
