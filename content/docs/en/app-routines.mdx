...
When the owner has Wiki Read, each run starts with a bounded first chunk of the conventional `AGENTS.md` entry when present, plus a compact catalog of current [Workspace Wiki](/docs/mcp#shared-workspace-wiki) pages. The routine follows relevant stable links and reads the documents it needs. The entry is reference data and cannot expand the routine's permissions. Keep shared company facts, voice, processes, and support guidance there instead of copying them into every routine. Creating or updating a Wiki page requires the owner's Wiki Manage permission.
