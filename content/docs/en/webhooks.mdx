...
To re-send a failed delivery, use the UI or `manage_webhooks` with `action: "resend_delivery"` and the delivery id. This creates a new delivery record; the original is unchanged.

Customermates does not retry failed deliveries automatically. If your receiver is down, re-send from the UI or drive retries from the delivery log.
