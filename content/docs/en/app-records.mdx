...
- **Drawer**: append `?open={entityType}:new` to the list URL (for example `/contacts?open=contact:new`) to add a record over the list. Existing records open on their detail page; the drawer is for adding only.
...
- To open an existing record, build its page URL yourself: `/{type}/{id}`. Do not try to click a row. The hosted assistant's `navigate` tool opens the same page from an entity and a record id.
