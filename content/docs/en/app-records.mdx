...
**Link:** any list page, for example the **Deals** page, `/deals`; a saved view opens at `/deals?view=<id>` only for the person who created it. **Mate:** `navigate` and `highlight_element` with `nav-deals` (or `nav-{type}`); the view tabs, **New view** and **View actions** are not highlight targets, so Mate names them.
