...
|---|---|---|---|
| Open the Company group | App sidebar | `#nav-company` | Expands the sidebar sub-items; the button itself does not navigate |
| Subscription tab | Sidebar sub-item | `#nav-company-subscription` | Plan, trial status, and the plan picker (Cloud only) |
| Settings tab | Sidebar sub-item | `#nav-company-settings` | Workspace currency, deal forecasting, and the data model |
...
| Roles list controls | Roles toolbar | `#company-roles-search` / `#company-roles-display-options` | List controls (roles have no filterable fields, so no filter control renders) |
...
| Audit list controls | Audit toolbar | `#company-audit-logs-search` / `#company-audit-logs-filter` / `#company-audit-logs-display-options` | List controls (search matches the record id only; the filter offers event and date) |

The display-options popovers expose these stable layout controls. None of these lists is groupable, so the board control renders disabled on every one of them:
