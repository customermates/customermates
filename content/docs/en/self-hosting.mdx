...
- `TRIGGER_SECRET_KEY` and `TRIGGER_PROJECT_REF`: a free [Trigger.dev](https://trigger.dev) project. Required for background webhook deliveries and lifecycle jobs.
...
Background jobs (webhook deliveries, trial lifecycle) run on Trigger.dev. Inspect their runs in the Trigger.dev dashboard.
