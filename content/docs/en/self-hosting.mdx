...
Both run the same core application. Enterprise features require a license in either deployment.
...
**Cost shape:** Cloud is priced per seat and scales linearly. Self-host is free for the core application plus whatever you pay for infrastructure (a small VPS handles hundreds of users). For a small team, cloud is often cheaper once you account for the time to run Postgres backups yourself. For a larger team, self-host is usually the lower cost.
