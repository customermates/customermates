...
- `BASE_URL`: your public origin (e.g. `https://crm.example.com`); use `http://localhost:4000` locally and include
  `APP_PORT` only when accessing that host port directly.
...
values. You do not configure a separate workflow database URL.
...
When the app is ready, open the configured `BASE_URL`.
...
The published image allows only same-origin framing. If you intentionally embed the app, configure the reverse
proxy to replace the upstream `Content-Security-Policy` header with your `frame-ancestors` policy. Do not append a
second CSP header; an additional policy cannot broaden the original one.
