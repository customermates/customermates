...
An entry stores the event, the entity id, the user it is attributed to, the timestamp, and the payload as JSON. Updates to records, roles, webhooks, custom fields, routines, and connected accounts carry a field-level diff. Creates and deletes carry a snapshot. A write that fails or changes nothing leaves no entry. Webhook secrets and header values are stripped before the event is stored. Other payloads carry record contents, so the log holds copies of the data it describes.

An action is recorded only if it publishes a domain event, and parts of the product publish none:
...
In the UI you can filter by event and date, search by entity id, and sort by time. There is no export. Nothing in the product edits or prunes entries, but they live in the same database as your data and are removed with the company or user they reference. Treat this as an operational record, not a tamper-evident compliance archive.
