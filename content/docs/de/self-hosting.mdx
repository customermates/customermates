...
- `POSTGRES_PASSWORD`: ein starkes, URL-sicheres Passwort.
- `BASE_URL`: Ihre öffentliche Origin (z.B. `https://crm.example.com`); lokal `http://localhost:4000` verwenden
  und `APP_PORT` nur beim direkten Zugriff auf diesen Host-Port einschließen.
...
Compose leitet die Datenbankverbindungen der Anwendung und des eingebetteten Workflow-Workers aus denselben
`POSTGRES_*`-Werten ab. Sie konfigurieren keine separate Workflow-Datenbank-URL.
...
Wenn die App ready ist, die konfigurierte `BASE_URL` öffnen.
...
Das veröffentlichte Image erlaubt Frames nur vom gleichen Origin. Wenn Sie die App bewusst einbetten, muss der
Reverse-Proxy den vorhandenen `Content-Security-Policy`-Header durch Ihre `frame-ancestors`-Policy ersetzen. Ein
zweiter CSP-Header erweitert die bestehende Policy nicht.
