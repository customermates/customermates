...
Ein Eintrag speichert das Ereignis, die Element-ID, die zugeordnete Person, den Zeitstempel und die Payload als JSON. Änderungen an Datensätzen, Rollen, Webhooks, benutzerdefinierten Feldern, Routinen und verbundenen Konten enthalten ein feldweises Diff. Erstellungen und Löschungen enthalten eine Momentaufnahme. Ein fehlgeschlagener Write und ein Write ohne Änderung hinterlassen keinen Eintrag. Webhook-Secrets und Header-Werte werden vor dem Speichern entfernt. Andere Payloads enthalten Datensatzinhalte, das Log hält also Kopien der Daten, die es beschreibt.

Eine Aktion wird nur protokolliert, wenn sie ein Domain-Event veröffentlicht, und Teile des Produkts veröffentlichen keines:
...
In der UI lässt sich nach Ereignis und Datum filtern, nach Element-ID suchen und nach Zeit sortieren. Einen Export gibt es nicht. Das Produkt bearbeitet oder löscht keine Einträge, sie liegen aber in derselben Datenbank wie Ihre Daten und werden mit dem zugehörigen Unternehmen oder Benutzer entfernt. Behandeln Sie dies als operatives Protokoll, nicht als manipulationssicheres Compliance-Archiv.

Audit-Logging ist kein Tarif-Feature. Jeder Cloud-Plan enthält es, ebenso die selbstgehostete Community-Edition: `features/audit-log/` ist AGPL-Code außerhalb von `ee/`. Wer es lesen darf, legt die Rolle über die Berechtigung `auditLog` fest.
