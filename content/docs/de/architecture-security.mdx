...
Den vollständigen Tool-Katalog findest du unter [MCP](/docs/mcp#tool-catalog).
...
- Webhook-Secrets liegen in Postgres, weil sie abrufbar sein müssen, um ausgehende Requests zu signieren. Wenn du sie woanders halten willst, self-hoste mit deinem eigenen Secret-Manager.

## Daten in-transit

- HTTPS überall in der Managed-Cloud.
- Bei Self-Host stellst du das TLS selbst bereit. Das Caddy-Beispiel findest du unter [Self-Hosting](/docs/self-hosting).
- Webhook-Deliveries gehen nur an HTTPS-URLs. HTTP wird auf Schema-Ebene abgelehnt.

## Audit-Logging (Enterprise)

Jeder Write wird mit User, Action, Entity sowie Vorher- und Nachher-Werten geloggt. Er ist aus der UI abfragbar und als JSON exportierbar. Audit-Logging ist ein Enterprise-Feature, verfügbar in kostenpflichtigen Cloud-Plänen oder beim Self-Host mit einem Lizenzschlüssel.

## Vulnerabilities melden

Bitte melde verantwortungsvoll an `security@customermates.com`. Der PGP-Key liegt im Repository. Wir bestätigen den Eingang innerhalb von 24 Stunden.
