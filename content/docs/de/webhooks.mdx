...
**Eigene Header** werden bei jeder Zustellung mitgesendet, für Endpunkte mit eigener Authentifizierung. In der Oberfläche tragen Sie sie zeilenweise als `Name: Wert` ein, über MCP und API als `headers`-Objekt. `Content-Type`, `Host` und `X-Webhook-Signature` setzt Customermates selbst und lassen sich nicht überschreiben; Header-Namen müssen gültige HTTP-Tokens sein, Werte müssen Latin-1-Text ohne Zeilenumbrüche sein. Da Header in der Regel ein Credential tragen, muss ein Webhook mit Headern einen HTTPS-Endpunkt verwenden.
...
## Zustellung an einen bestimmten Dienst
