...
|---|---|---|---|---|
| **Abonnement** | `/company/subscription` | `nav-company-subscription` | `company-subscription-manage`, `company-subscription-refresh`, `company-subscription-plan-picker` | [Abonnement](#subscription-tab) |
| **Einstellungen** | `/company/settings` | `nav-company-settings`, auch `nav-company` | `company-settings-currency`, `company-settings-deal-stage-field`, `company-settings-stage-weights`, `company-settings-total-pipeline`, `company-settings-weighted-pipeline`, `company-settings-data-model`, `terminology-contact`, `terminology-organization`, `terminology-deal`, `terminology-service`, `terminology-task`, `company-settings-save`, `company-settings-reset` | [Einstellungen](#settings-tab) |
| **Mitglieder** | `/company/members` | `nav-company-members` | `company-members-add`, `company-members-search`, `company-members-filter`, `company-members-display-options`, `company-members-layout-table`, `company-members-layout-board`; in **Benutzer einladen** `invite-modal-tab-link`, `invite-modal-link`, `invite-modal-copy-link`, `invite-modal-tab-email`, `invite-modal-emails`, `invite-modal-send`; in **Benutzer** `member-modal-email`, `member-modal-first-name`, `member-modal-last-name`, `member-modal-country`, `member-modal-role`, `member-modal-avatar-url`, `member-modal-status`, `member-modal-save`, `member-modal-reset` | [Mitglieder](#members-tab) |
| **Rollen** | `/company/roles` | `nav-company-roles` | `company-roles-add`, `company-roles-display-options`, `company-roles-layout-table`, `company-roles-layout-board`; im Editor `role-modal-save`, `role-modal-reset`, `role-modal-delete` | [Rollen](#roles-tab) |
...
| **Letzte Zustellungen** | `/company/webhook-deliveries` | `nav-company-webhook-deliveries` | `company-webhook-deliveries-search`, `company-webhook-deliveries-filter`, `company-webhook-deliveries-display-options`, `company-webhook-deliveries-layout-table`, `company-webhook-deliveries-layout-board`; in **Ereignis-Details** `webhook-delivery-modal-resend` | [Letzte Zustellungen](#deliveries-tab) |
| **Abonnement erforderlich** | `/subscription-expired` | Keines | Keine, Mate ist dort nicht verfügbar | [Ende der Testphase oder ausbleibende Zahlung](#what-happens-when-the-trial-ends-or-a-payment-fails) |
| **Rechtsdokumente aktualisiert** | `/legal-update` | Keines | Keine, Mate ist dort nicht verfügbar | [Mitglieder ohne Berechtigung](#what-does-a-member-without-a-permission-see) |
...
**Link:** `/legal-update` für **Rechtsdokumente aktualisiert**; die Seite öffnet sich nur, solange eine Zustimmung aussteht. **Mate:** kein Ziel, und Mate ist auf dieser Seite nicht verfügbar, kann also nur den Link nennen; das Kontrollkästchen `#legal-acceptance` ist eine Element-Id, die Mate nicht hervorheben kann.
...
**Link:** die Seite **Abonnement**, `/company/subscription`. **Mate:** `navigate` und `highlight_element` mit `nav-company-subscription`; `highlight_element` nimmt außerdem `company-subscription-refresh`, `company-subscription-manage` und `company-subscription-plan-picker` an, jeweils nur sichtbar, wenn Rolle und Tarif sie zeigen (siehe unten). Die Felder sind keine Highlight-Ziele, deshalb nennt Mate sie beim Namen. Selbstgehostet bietet `list_ui_targets` die Ziele weiterhin an, die Seite leitet aber zum Dashboard um.
...
**Link:** `/company/subscription`. **Mate:** `navigate` und `highlight_element` mit `nav-company-subscription`, danach `highlight_element` mit `company-subscription-manage` für **Verwalten mit Lemon Squeezy** oder `company-subscription-refresh` für **Aktualisieren**; beide sind nur für die oben genannten Rollen und Tarife sichtbar.
...
**Link:** die Seite **Audit-Logs**, `/company/audit-logs`. **Mate:** `navigate` und `highlight_element` mit `nav-company-audit-logs`; `highlight_element` nimmt außerdem die Toolbar-Ids aus dieser Tabelle an. Zeilen und **Audit-Log-Details** sind keine Highlight-Ziele, deshalb nennt Mate sie beim Namen.
