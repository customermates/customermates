...
In Customermates: **Profil → API Keys → Neuer Key**. Geben Sie ihm einen Namen, der den Client beschreibt (z.B. `Claude Code`). Der 64-Zeichen-Key wird einmal angezeigt, also sofort kopieren. Ein Key erbt die Berechtigungen des Users, der ihn erstellt hat; ein Scope pro Key gibt es nicht. Legen Sie pro Client einen eigenen Key an, wenn Sie Audit-Log-Granularität wollen.
...
Der Server stellt beim Verbinden Anweisungen zur Tool-Oberfläche, zu Sicherheitsregeln und zum Abruf relevanter Workspace-Wiki-Seiten bereit. Der jeweilige Client entscheidet, ob er diese Anweisungen an seinen Agent weitergibt. Tut er das nicht, ergänzen Sie die Wiki-Abrufregel in den dauerhaften Anweisungen dieses Clients. In Clients, die MCP-Prompts unterstützen (etwa Claude Code), können Sie den eingebauten `get-started`-Prompt für einen geführten Start nutzen: Er befragt Sie und fasst dann Ihren Workspace zusammen.
