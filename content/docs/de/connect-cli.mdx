...
description: Verbinde Claude Code, Codex, Cursor oder die Gemini CLI per API-Key mit Customermates, dem Open-Source-KI-CRM. Ein Abschnitt pro Client, plus Konfigurationsdatei-Fallback für Claude Desktop.
---
...
> Nutzt du stattdessen die **App** von Claude oder ChatGPT? Die verbinden sich mit [dem Custom Connector](/docs/connect-custom-connector) (OAuth, kein Key).
...
Falls `mcpServers` bereits Einträge enthält, füg `customermates` daneben ein, statt das Objekt zu überschreiben.

## Claude Desktop (config file)

Der empfohlene Weg für Claude Desktop ist der [Connector-Weg](/docs/connect-custom-connector#claude): OAuth, kein Key, über deine Geräte synchronisiert. Im Free-Plan, oder wenn du lieber einen statischen Key nutzt, funktioniert die Konfigurationsdatei unten auch.
...
`mcp-remote` ist ein Shim, der Claude Desktop über stdio mit einem Remote-HTTP-MCP-Server sprechen lässt. Er wird beim ersten Aufruf via `npx` heruntergeladen, du brauchst also Node 18+ im `PATH`.
