...
| `manage_data_views` |  | ✓ | Ein Tool für persönliche gespeicherte Ansichten: surfaces, config, list, create, update, select, delete. Vor dem Schreiben verfügbare Oberflächen sowie unterstützte Filter, Suche, Sortierung, Gruppierung und Layouts ermitteln. Create wählt die neue Ansicht aus; Updates behalten ausgelassene Einstellungen und das Spaltenlayout bei. viewKey `__all__` bezeichnet Alle. Das Löschen einer Ansicht löscht keine Datensätze. Zurückgegebene Links öffnen Ansichten; externe Änderungen erscheinen nach dem Aktualisieren der Seite. Operator-Ansichten benötigen eine interaktive Operator-Sitzung. |
{/* /mcp-catalog */}
