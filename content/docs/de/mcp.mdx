...
- **Explizite Spaltenabsicht**: Neue Integrationen legen mit `intent=create` ohne ID an; bestehende Aufrufe ohne ID bleiben reine Anlegevorgänge. Aktualisierungen brauchen `intent=update` plus eine vorhandene ID. Für `singleSelect` wird die vollständige Auswahlliste im obersten Feld `selectOptions` übergeben; das bisherige `options.options` bleibt kompatibel, beide Formen dürfen aber nicht gleichzeitig vorkommen. `label`, `type` und `entityType` bleiben beim Update unverändert, daher erfordert eine andere Bezeichnung oder ein anderer Typ eine neue Spalte.
...
| Tool                    | Read | Destructive | Zweck                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| ----------------------- | ---- | ----------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `manage_custom_columns` |      | ✓           | Ein Tool mit `action`-Schalter: list, upsert (anlegen oder aktualisieren), delete. Neue Integrationen nutzen `intent=create` ohne ID; bestehende Aufrufe ohne ID bleiben als Anlegevorgänge kompatibel. Aktualisierungen brauchen `intent=update` mit vorhandener ID. Für `singleSelect` wird `selectOptions` auf oberster Ebene bevorzugt; das bisherige `options.options` bleibt kompatibel. Deckt alle zehn Spaltentypen ab; label, type und entityType sind beim Update unveränderlich. Delete ist IRREVERSIBLE und löscht alle gespeicherten Werte. |
...
| Tool                        | Read | Destructive | Zweck                                                                                                                                                                                    |
| --------------------------- | ---- | ----------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `update_workspace_settings` |      |             | `target` profile aktualisiert Ihren Namen, Ihr Land und Ihren Avatar; `target` company aktualisiert die Workspace-Währung und die voreingestellten Namen der Kernentitäten (nur Admins). |
...
| Tool              | Read | Destructive | Zweck                                                                                                                                                                                                                                                                                                  |
| ----------------- | ---- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `request_support` |      |             | Sendet eine Support-Anfrage per E-Mail an das Customermates-Team (Betreff plus Beschreibung). Nehmen Sie alle relevanten Details in die Beschreibung auf. Das Team antwortet an die E-Mail-Adresse Ihres Kontos. Gibt eine Bestätigung zurück, nachdem der E-Mail-Anbieter die Anfrage angenommen hat. |
