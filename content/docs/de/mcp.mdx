...
| Tool | Read | Destructive | Zweck |
|---|---|---|---|
| `get_social_posts` | ✓ | | Posts auf LinkedIn oder Instagram, gelesen über ein verbundenes Konto. Listet die Posts einer Person über authorIdentifier (`me` oder die oberste Profil-/Teilnehmer-`id`; nie public_identifier oder member_id) oder holt einen Post per postId. Beim ersten Aufruf cursor und offset weglassen, danach next_cursor weitergeben; LinkedIn-Nutzerposts verwenden Cursor-Paginierung. |
...
| `get_social_profile` | ✓ | | Ein Personen- oder Unternehmensprofil. Für Personen `profileType=person` mit `me`, der obersten Profil-/Teilnehmer-`id`, einem LinkedIn-Classic-public_identifier oder einem Instagram-Nutzernamen verwenden; nie member_id. Für LinkedIn-Unternehmen `profileType=company` mit der Unternehmens-ID verwenden. |
