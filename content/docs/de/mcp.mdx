...
Sobald die Anfrage angenommen wurde, geht das Onboarding weiter, während eine sichtbare Unterhaltung mit dem Titel **Set up Workspace Wiki** läuft. Mate liest die angegebene Seite direkt und bei Bedarf bis zu vier hilfreiche Seiten, die von dieser Homepage auf derselben Domain oder ihren Subdomains verlinkt sind. Daraus entstehen eine bis fünf nützliche Wiki-Seiten in Ihrer Oberflächensprache, mit Quell-Links und gekennzeichneten Informationslücken statt erfundenen Angaben. Die Einrichtung nutzt Ihr normales gehostetes KI-Guthaben.
...
Verbundene KI-Clients lesen dieselben aktuellen Seiten über diesen MCP-Endpunkt. Beginnen Sie mit `get_workspace_context`: Der `wiki`-Katalog enthält zehn Titel und kurze Auszüge vom Seitenanfang. Die Auszüge helfen bei der Auswahl; lesen Sie die Seite, bevor Sie ihre Inhalte als Vorgabe verwenden. Übergeben Sie `wiki.nextPage` als `wikiPage`, um den Katalog fortzusetzen.
...
Der Server weist verbundene Agents an, relevante Wiki-Seiten zu lesen, bevor sie Unternehmensfakten, Abläufe, Sprache oder Support-Vorgaben verwenden. Wann der Kontext abgerufen wird, entscheidet der externe Client. Eine Verbindung garantiert daher keinen Wiki-Abruf bei jeder Anfrage. Falls Ihr Client Server-Anweisungen ignoriert, ergänzen Sie den gleichen Abrufhinweis in seinen dauerhaften Anweisungen. Die Dokumentation selbst bleibt im CRM, damit alle Clients aktuelle Inhalte lesen.
...
| Tool | Read | Destructive | Zweck |
|---|---|---|---|
| `get_workspace_context` | ✓ |  | Benutzer, Unternehmen, Wiki-Katalog, Rollen und verbundene Messaging-Konten in einem Aufruf. Der Wiki-Katalog enthält zehn Seitentitel und Auszüge; für weitere Seiten nextPage als wikiPage übergeben. Ohne Wiki-Leserechte werden keine Wiki-Daten ausgegeben. |
