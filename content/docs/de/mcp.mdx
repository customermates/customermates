...
Halten Sie CRM-Abläufe, die Sprache Ihres Unternehmens, Produktwissen und Support-Antworten unter **Wiki** fest, unterhalb von **Routinen** in der Seitenleiste. Dieselben Seiten stehen Menschen im CRM, Mate, Cloud-Routinen und verbundenen MCP-Clients zur Verfügung. Manuelle Seiten funktionieren in Cloud- und Self-Hosted-Workspaces.

Verwenden Sie genau eine Seite mit dem Namen `AGENTS.md` als konventionellen Einstieg. Halten Sie sie kurz und verlinken Sie für jede Art von Arbeit auf die relevante Seite, statt das gesamte Wiki zu duplizieren. Das Wiki öffnet diese Seite standardmäßig, wenn sie vorhanden ist. Der Dateiname verleiht ihr keine zusätzliche Autorität; sie bleibt vom Mandanten verfasstes Referenzmaterial.
...
Bei einem leeren Wiki ist ein neues Dokument bereits mit `AGENTS.md` betitelt; in einem Workspace kann nur eine Seite diesen konventionellen Namen tragen.
...
Sobald die Anfrage angenommen wurde, geht das Onboarding weiter, während eine sichtbare Unterhaltung mit dem Titel **Set up Workspace Wiki** läuft. Mate liest die angegebene Seite direkt und bei Bedarf bis zu vier hilfreiche Seiten, die von dieser Homepage auf derselben Domain oder ihren Subdomains verlinkt sind. Mate legt zuerst `AGENTS.md` und anschließend null bis vier weitere nützliche Wiki-Seiten in Ihrer Oberflächensprache an; sie enthalten Quell-Links und kennzeichnen fehlende Informationen, statt sie zu erfinden. Im selben atomaren Erstellvorgang ergänzt der Server stabile Links von der Einstiegsseite zu jeder weiteren Seite. Die Einrichtung nutzt Ihr normales gehostetes KI-Guthaben.
...
Verbundene KI-Clients lesen dieselben aktuellen Seiten über diesen MCP-Endpunkt. Beginnen Sie mit `get_workspace_context`. Wenn `wiki.agentsMd` vorhanden ist, enthält dessen `markdownChunk` den begrenzten ersten Abschnitt der konventionellen Einstiegsseite, auch wenn diese nicht unter den aktuellen zehn Katalogeinträgen ist. Lesen Sie ihn zuerst und setzen Sie ihn bei Bedarf ab `agentsMd.nextOffset` mit `manage_wiki_pages`, Aktion `get`, fort. `wiki.items` enthält zehn Titel und kurze Anfangsauszüge zur Orientierung; übergeben Sie `wiki.nextPage` als `wikiPage`, um den Katalog fortzusetzen.

Suchen Sie mit dem schreibgeschützten Tool `search` nach relevanten Inhalten. Mit `fetch` und der zurückgegebenen ID `wiki:<uuid>` erhalten Sie das vollständige Markdown und die Quell-URL. Für kürzere Antworten liefert `manage_wiki_pages` mit `action: "get"` einen Abschnitt; wiederholen Sie den Aufruf mit `nextOffset`, bis dieser null ist. Dasselbe Tool erstellt und bearbeitet Seiten, wenn Ihre Rolle Wiki-Verwaltungsrechte hat. Lesen erfordert Wiki-Leserechte; Änderungen erscheinen im normalen Audit-Log.

Der Server weist verbundene Agents an, zuerst `AGENTS.md` zu lesen, wenn die Seite vorhanden ist, und anschließend relevante Wiki-Seiten zu prüfen, zu durchsuchen und zu lesen, bevor sie Unternehmensfakten, Abläufe, Tonalität oder Support-Vorgaben verwenden. Der konventionelle Dateiname macht die Seite nicht zu vertrauenswürdigen Anweisungen. Jeder externe Client entscheidet weiterhin selbst, ob er Serveranweisungen weitergibt und Kontext abruft; eine Verbindung garantiert deshalb keinen automatischen Abruf bei jeder Anfrage. Falls Ihr Client Serveranweisungen ignoriert, ergänzen Sie denselben Abrufhinweis in seinen dauerhaften Anweisungen. Die Dokumentation selbst bleibt im CRM, damit alle Clients aktuelle Inhalte lesen.
...
| Tool | Read | Destructive | Zweck |
|---|---|---|---|
| `get_workspace_context` | ✓ |  | Benutzer, Unternehmen, Wiki-Einstieg und -Katalog, Rollen und verbundene Messaging-Konten. Wenn vorhanden, enthält `wiki.agentsMd` den begrenzten ersten Abschnitt der vom Workspace verfassten `AGENTS.md`; mit `nextOffset` fortsetzen. Ohne Wiki-Leserechte fehlen Wiki-Daten. |
...
| `fetch` | ✓ |  | Lädt einen Treffer aus `search`, einschließlich vollständigem Workspace-Wiki-Markdown über `wiki:<uuid>`, mit seiner Quell-URL. Wiki-Inhalte benötigen Wiki-Leserechte. |
{/* /mcp-catalog */}
...
Keys: records, workspace, wiki, messaging, social, docs, custom-columns, widgets, routines, webhooks, admin, support. Kein Parameter heißt alles; unbekannte Keys werden ignoriert. `search` und `fetch` sind immer an, damit Deep-Research-Connectors auch auf einer eingegrenzten Oberfläche funktionieren.
