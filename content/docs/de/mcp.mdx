...
Pflicht: `entity`. Optional: `searchTerm`, `filters`, `sortDescriptor`, `page`, `pageSize`.

#### `search_records`

Freitextsuche über einen oder mehrere Entity-Typen in einem Call.

Pflicht: `searchTerm`. Optional: `entities`, `limitPerEntity`.
...
Optional: `threadId`, `page`, `pageSize`, `searchTerm`, `filters`, `sortDescriptor`.

#### `get_activities`

Aktivitäts-Timeline mit optionalem technischem Entitätsumfang und UND-verknüpften Filtern. Filter unterstützen Kategorie/Roh-Typ, Unterhaltung, Anbieter, verbundenen Account sowie verknüpfte Kontakte, Organisationen, Deals, Services und Aufgaben. Jedes Aktivitätsfilterfeld darf einmal vorkommen; Alternativen gehören in das Werte-Array einer einzigen Mitgliedschaftsregel. Beziehungsfelder akzeptieren `in`, `notIn`, `hasSome` und `hasNone`; Mitgliedschaft benötigt 1–50 UUIDs. Beziehungs-UUIDs müssen auf Datensätze verweisen, die Sie lesen dürfen; nicht auflösbare Ids werden abgelehnt. Das Ergebnis enthält `availableSources`, `scopeTruncated`, `pageLimitReached`, Gesamtzahl und Seite. Die Seitennummer ist auf 40 begrenzt.

Optional: `page`, `pageSize`, `scope`, `filters`, `sortDescriptor`.
...
Optional: `list`, `eventId`, `searchTerm`, `filters`, `sortDescriptor`, `page`, `pageSize`.
...
Pflicht: `messageId`, `draftRevision`.
...
Pflicht: `connectedAccountId`. Optional: `postId`, `authorIdentifier`, `cursor`, `offset`, `limit`.
...
Pflicht: `connectedAccountId`, `postId`. Optional: `kind`, `commentId`, `sortBy`, `cursor`, `offset`, `limit`.

#### `get_social_profile`

Ein Personen- oder Unternehmensprofil. Für Personen `profileType=person` mit `me`, `get_messaging_threads.items[].participants[].identifier`, `get_messaging_threads.thread.participants[].identifier`, `get_social_posts.items[].author.id`, `get_social_posts.author.id`, `get_social_post_engagement.items[].author.id`, `get_social_post_engagement.items[].sender.id`, `manage_social_relations.items[].user.id`, einem öffentlichen LinkedIn-Classic-Profil-Slug oder einem Instagram-Nutzernamen verwenden. Für LinkedIn-Unternehmen `profileType=company` mit `linkedin_search_sales_companies.items[].id`, `linkedin_search_sales_leads.items[].current_positions[].company_id`, `linkedin_manage_sales_lists.items[].current_positions[].company_id` oder `get_social_profile.current_positions[].company_id` verwenden. `get_social_profile.id` mit demselben `profileType` wiederverwenden.

Pflicht: `connectedAccountId`, `identifier`. Optional: `profileType`.
...
Pflicht: `connectedAccountId`. Optional: `url`, `filters`, `offset`, `limit`.
...
Pflicht: `connectedAccountId`, `type`. Optional: `keywords`, `offset`, `limit`.
...
| Tool | Read | Destructive | Zweck |
|---|---|---|---|
| `manage_custom_columns` |  | ✓ | Ein Tool mit `action`-Schalter: list, upsert (anlegen oder aktualisieren), delete. upsert verlangt `intent` (`create` oder `update`). Deckt alle zehn Spaltentypen ab; label, type und entityType sind beim Update unveränderlich. Delete ist IRREVERSIBLE und löscht alle gespeicherten Werte. |
...
Pflicht: `action`. Optional: `entityType`, `id`, `intent`, `type`, `label`, `selectOptions`, `options`.
{/* /mcp-catalog */}
...
Pflicht: `action`. Optional: `kind`, `id`, `ids`, `name`, `entityType`, `entityFilters`, `dealFilters`, `displayType`, `groupByType`, `groupByCustomColumnId`, `aggregationType`, `reverseXAxis`, `reverseYAxis`, `barColors`, `timelineFilters`, `showFilters`.
{/* /mcp-catalog */}
...
Pflicht: `action`. Optional: `id`, `cursor`, `page`, `pageSize`, `searchTerm`, `name`, `prompt`, `enabled`, `triggerKind`, `cronExpression`, `timezone`, `triggerEvents`, `changedFields`, `triggerFilters`, `debounceSeconds`.
{/* /mcp-catalog */}
...
Pflicht: `action`. Optional: `id`, `url`, `description`, `events`, `secret`, `headers`, `bodyTemplate`, `enabled`, `searchTerm`, `filters`, `sortDescriptor`, `page`, `pageSize`.
{/* /mcp-catalog */}
