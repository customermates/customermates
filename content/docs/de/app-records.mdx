...
- **Liste** unter `/{type}` (zum Beispiel `/contacts`), locale-präfixiert wie `https://customermates.com/de/contacts`. Suche, Filter, Sortierung, gespeicherte Ansichten und Massenaktionen leben hier.
...
**Link:** jede Listenseite, zum Beispiel die Seite **Deals**, `/deals`, Cloud-URL `https://customermates.com/de/deals`; eine gespeicherte Ansicht öffnet sich unter `/deals?view=<id>` nur für die Person, die sie angelegt hat. **Mate:** `navigate` und `highlight_element` mit `nav-deals` (oder `nav-{type}`); die Ansichts-Tabs, **Neue Ansicht** und **Ansichtsaktionen** sind keine Highlight-Ziele, deshalb nennt Mate sie beim Namen.
...
**Link:** eine Detailseite, `/contacts/<id>` (ebenso `/organizations/<id>`, `/deals/<id>`, `/services/<id>`, `/tasks/<id>`), Cloud-URL `https://customermates.com/de/contacts/<id>`. **Mate:** `navigate` mit der Entität und ihrer recordId; die Topbar-Buttons, der Drawer, die Auswahlleiste, **Aktualisieren** und **Ausgewählte löschen** sind keine Highlight-Ziele, deshalb nennt Mate sie beim Namen.
