...
- Das `list_records` MCP-Tool (und die Messaging-Filter auf `get_messaging_threads`).
...
Datumsfelder wie `createdAt` und `updatedAt` akzeptieren `gt`, `gte`, `lt`, `lte`, `between` und `inLastDays`. Nutze `inLastDays` mit einer ganzzahligen Anzahl von Tagen für ein rollierendes Aktualitätsfenster, zum Beispiel `{ "field": "createdAt", "operator": "inLastDays", "value": 7 }` für Datensätze der letzten Woche.
...
- Relationship-Arrays (`organizationIds`, `dealIds`, `userIds`, `contactIds`). Kombiniere sie mit `in`, `notIn`, `hasNone`, `hasSome`.
- Custom-Column-IDs. Nutze die UUID der Spalte als Feldname.

Immer `get_record_schema` zuerst, wenn du unsicher bist, was filterbar ist. Die Fehlermeldung bei unbekanntem Feld listet alle verfügbaren Felder mit erlaubten Operatoren.
