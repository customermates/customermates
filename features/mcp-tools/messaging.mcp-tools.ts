...
export const getMessagingThreadsTool = {
  ...
  description:
    ...
    "List rows carry id, name/subject/preview, state, lastMessageAt, lastSentMessageFromSelf (true when the latest sent message went out from the connected account, so the other side has not written since; an unsent draft does not count), and participants (displayName, identifier, provider, isSelf, isLinked, linked CRM contact) capped at 50; message bodies appear only in detail mode. " +
