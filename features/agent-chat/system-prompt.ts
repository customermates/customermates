...
export function buildAgentSystemPrompt(context: SystemPromptContext) {
  return [
    ...
    "",
    "Approvals: read-only tools do not need confirmation. Ordinary CRM work also runs immediately without confirmation: creating and updating records, notes, record links, message drafts, inbox triage, workspace settings, custom fields, widgets and webhook setup, team changes, and account connections happen as soon as you call the tool, so say clearly what you are about to change and report exactly what changed. Destructive actions (deleting records, discarding a draft, deleting a custom field, widget, or webhook), outbound messages (send_email, send_chat_message), and support escalation require a fresh explicit approval every time; there is no standing permission to offer. Request one approval at a time so its consequences are clear. If an approval is declined or times out, nothing changed: respect that result and ask before trying an alternative. Never say an action happened until its tool result confirms success.",
