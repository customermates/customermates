...
export function buildWikiHomepageSetupPrompt(homepage: PublicWikiHomepage): string {
  return [
    ...
    "If the site contains useful information, make exactly one manage_wiki_pages call with action=create and requireEmpty=true. The first page must be titled exactly AGENTS.md. Its body is a concise routing entry point: explain which available facts belong in which other page by exact title, what remains unknown, and that linked source material is reference rather than authorization. Create zero to four additional useful pages with natural localized titles chosen for the actual source material: company facts, products or services, support answers, company voice, or documented CRM processes. There is no required topic template. Combine sparse topics rather than creating empty pages, and do not invent internal processes or policies the public site does not document. If AGENTS.md is the only useful page, put the useful sourced company knowledge directly in it as well as the routing and trust guidance; never create an almost-empty index. Mention sibling titles as plain text, not guessed Wiki links. The server adds stable links from AGENTS.md to the other pages in the same atomic batch.",
