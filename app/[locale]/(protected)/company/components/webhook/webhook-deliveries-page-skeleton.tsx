...
export function WebhookDeliveriesPageSkeleton({ animated = true, view = "table" }: Props) {
  const spec: DataViewSkeletonSpec = view === "table" ? { tableVariant: "plain", view } : { identity: "text", view };
  return <DataViewSkeleton data-webhook-deliveries-page-skeleton animated={animated} spec={spec} />;
}
