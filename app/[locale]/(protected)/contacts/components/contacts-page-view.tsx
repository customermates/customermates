...
import { resolveDataViewPageState, resolveDataViewSkeletonView } from "@/components/data-view/data-view-state";
...
export const ContactsPageView = observer(function ContactsPageView({ contacts }: Props) {
  ...
  const view = resolveDataViewSkeletonView(contactsStore.viewMode, contactsStore.groupingColumnId);
  const hasActiveQuery = Boolean(contactsStore.searchTerm?.trim()) || (contactsStore.filters?.length ?? 0) > 0;
  const pageState = resolveDataViewPageState({
    explicitlyUnpaginated: false,
    failure: contactsStore.refreshError !== null,
    hasActiveQuery,
    hasUsableContent: contactsStore.items.length > 0,
    isReady: contactsStore.isReady,
    isRefreshing: contactsStore.isRefreshing,
