...
import { AuditActionLabel } from "../components/operator-value-labels";
...
describe("operator audit action labels", () => {
  ...
  it("keeps the catalog aligned with the audited actions", () => {
    expect(Object.keys(en.OperatorAudit.values.action).sort()).toEqual(entries.map(([name]) => name).sort());
  });
