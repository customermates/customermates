...
const actionsMock = vi.hoisted(() => ({
  ...
  cleanupAgentWorkspaceSetupAction: vi.fn(),
...
async function prepareWorkspaceSetup(store: AgentChatStore, commandId = "setup-command") {
  (
    store as unknown as {
  ...
  await vi.waitFor(() =>
    expect(store.items.some((item) => item.kind === "workspace_setup" && item.commandId === commandId)).toBe(true),
  );
  const item = store.items.find(
    (candidate): candidate is Extract<(typeof store.items)[number], { kind: "workspace_setup" }> =>
      candidate.kind === "workspace_setup" && candidate.commandId === commandId,
  );
  if (!item) throw new Error("Workspace setup fixture was not prepared.");
  return item;
}
...
describe("AgentChatStore", () => {
  ...
  it("edits a workspace setup by prefilling the composer without sending", () => {
    const store = new AgentChatStore(root() as never);
    store.isHistoryOpen = true;

    store.editWorkspaceSetup("Please ask what I want to adjust.");

    expect(store.composerDraft).toBe("Please ask what I want to adjust.");
    expect(store.isHistoryOpen).toBe(false);
    expect(store.items).toEqual([]);
  });

  it("locks transcript-changing interactions while a workspace setup action is pending", async () => {
    const store = new AgentChatStore(root() as never);
    const originalConversationId = "00000000-0000-4000-8000-000000000001";
    store.conversationId = originalConversationId;
    store.composerDraft = "Do not send";
    store.queuedPrompt = "Keep queued";
    const approval = {
      kind: "approval",
      id: "approval",
    ...
    store.items = [{ kind: "workspace_setup", id: "setup", pendingAction: "apply" } as never, approval];

    store.submitDraft();
    store.setComposerDraft("Still do not send");
    store.editQueuedPrompt();
    store.newConversation();
    await store.selectConversation("00000000-0000-4000-8000-000000000002");
    await store.archiveConversation(originalConversationId);
    await store.restoreArchivedConversation("00000000-0000-4000-8000-000000000003");
    await store.respondToApproval(approval, "approve");

    expect(store.isWorkspaceSetupPending).toBe(true);
    expect(store.conversationId).toBe(originalConversationId);
    expect(store.composerDraft).toBe("Do not send");
    expect(store.queuedPrompt).toBe("Keep queued");
    expect(actionsMock.getAgentConversationAction).not.toHaveBeenCalled();
    expect(actionsMock.archiveAgentConversationAction).not.toHaveBeenCalled();
    expect(actionsMock.restoreAgentConversationAction).not.toHaveBeenCalled();
    expect(actionsMock.respondToApprovalAction).not.toHaveBeenCalled();
  });
  ...
  it("turns a validated setup command into one deterministic review card before acknowledging it", async () => {
    const store = new AgentChatStore(root() as never);
    store.conversationId = "00000000-0000-4000-8000-000000000001";
    ...
    expect(store.items.filter((item) => item.kind === "workspace_setup")).toEqual([
      expect.objectContaining({
        id: expect.any(String),
        commandId: "setup-command-1",
        status: "ready",
        plan: expect.objectContaining({ useCase: "clientProjects" }),
      }),
    ]);
  });
  ...
  it("builds the setup review in the current German locale", async () => {
    ...
    (
      ...
      seq: 1,
      type: "ui_command",
      commandId: "setup-command-de",
      name: "open_workspace_setup",
      input: {
        useCase: "b2bSales",
        businessName: "Beispiel GmbH",
        goal: "Eine Vertriebspipeline einrichten",
      },
    });
  ...
  it("supersedes an older live setup review when a newer plan is prepared", async () => {
    const store = new AgentChatStore(root() as never);
    store.conversationId = "00000000-0000-4000-8000-000000000001";
    const handleEvent = (
      store as unknown as {
    ...
    await vi.waitFor(() =>
      expect(store.items.some((item) => item.kind === "workspace_setup" && item.commandId === "setup-command-1")).toBe(
        true,
      ),
    );
    ...
    handleEvent({
      seq: 2,
      type: "ui_command",
      commandId: "setup-command-2",
      name: "open_workspace_setup",
      input: { ...setupInput, goal: "Track a revised delivery process" },
    });
    await vi.waitFor(() =>
      expect(store.items.some((item) => item.kind === "workspace_setup" && item.commandId === "setup-command-2")).toBe(
        true,
      ),
    );
  ...
  it("applies exactly one reviewed setup and refreshes every affected workspace surface", async () => {
    const conversationId = "00000000-0000-4000-8000-000000000021";
    const setupId = "00000000-0000-4000-8000-000000000022";
    let resolveApply!: (value: {
      ok: true;
      data: {
        status: "applied";
        setupId: string;
        counts: { columns: number; records: number; widgets: number };
      };
    }) => void;
    actionsMock.applyAgentWorkspaceSetupAction.mockImplementationOnce(
      () =>
        new Promise((resolve) => {
    ...
    const store = new AgentChatStore(rootStore as never);
    store.conversationId = conversationId;
    const item = await prepareWorkspaceSetup(store);

    const first = store.applyWorkspaceSetup(item);
    const duplicate = store.applyWorkspaceSetup(item);

    expect(item.pendingAction).toBe("apply");
    expect(actionsMock.applyAgentWorkspaceSetupAction).toHaveBeenCalledOnce();
    await duplicate;
    resolveApply({
      ok: true,
      data: {
        status: "applied",
        setupId,
        counts: { columns: 1, records: 5, widgets: 1 },
      },
    });
    await first;

    expect(actionsMock.applyAgentWorkspaceSetupAction).toHaveBeenCalledWith({
      conversationId,
    ...
    expect(rootStore.contactsStore.refresh).toHaveBeenCalledOnce();
    expect(rootStore.organizationsStore.refresh).toHaveBeenCalledOnce();
    expect(rootStore.dealsStore.refresh).toHaveBeenCalledOnce();
    expect(rootStore.servicesStore.refresh).toHaveBeenCalledOnce();
    expect(rootStore.tasksStore.refresh).toHaveBeenCalledOnce();
    expect(rootStore.widgetsStore.refresh).toHaveBeenCalledOnce();
    expect(rootStore.terminologyStore.refresh).toHaveBeenCalledOnce();
  });

  it("maps safe cleanup summaries and exposes full cleanup only for edited retained setup data", async () => {
    const conversationId = "00000000-0000-4000-8000-000000000023";
    const setupId = "00000000-0000-4000-8000-000000000024";
    actionsMock.cleanupAgentWorkspaceSetupAction
    ...
    await store.cleanupWorkspaceSetup(item, "safe");

    expect(actionsMock.cleanupAgentWorkspaceSetupAction).toHaveBeenNthCalledWith(1, {
      conversationId,
      setupId,
      planHash: item.planHash,
      mode: "safe",
    ...
    await store.cleanupWorkspaceSetup(item, "full");

    expect(actionsMock.cleanupAgentWorkspaceSetupAction).toHaveBeenNthCalledWith(2, {
      conversationId,
      setupId,
      planHash: item.planHash,
      mode: "full",
  ...
  it("never offers full cleanup for setup data retained only because it is still linked", async () => {
    const store = new AgentChatStore(root() as never);
    store.conversationId = "00000000-0000-4000-8000-000000000025";
    const item = await prepareWorkspaceSetup(store);
    item.status = "partiallyCleaned";
    item.setupId = "00000000-0000-4000-8000-000000000026";
    item.cleanupSummary = {
      deletedResources: 4,
      retainedResources: 1,
      missingResources: 0,
    ...
    expect(actionsMock.cleanupAgentWorkspaceSetupAction).not.toHaveBeenCalled();
    expect(item.status).toBe("partiallyCleaned");
  });
  ...
  it("supersedes an older setup loaded upward when a newer review is already visible", async () => {
    ...
    const store = new AgentChatStore(root() as never);
    store.conversationId = conversationId;
    const newer = await prepareWorkspaceSetup(store, "setup-newer");
    store.olderMessagesCursor = "older-page";
    actionsMock.getAgentConversationAction.mockResolvedValueOnce({
      id: conversationId,
      title: "Setup chat",
      messages: [
        {
          id: "message-old-setup",
          role: "assistant",
          parts: [
            {
    ...
    const setups = store.items.filter((item) => item.kind === "workspace_setup");
    expect(setups).toHaveLength(2);
    expect(setups[0]).toMatchObject({
      commandId: "setup-older",
      status: "superseded",
    });
