...
async function verify() {
  ...
  if (!apiKey)
    throw new Error(
      "LEMONSQUEEZY_API_KEY is required for explicit provider verification",
    );
  if (!storeId)
    throw new Error(
      "LEMONSQUEEZY_STORE_ID is required for explicit provider verification",
    );
  ...
  const bindingEnvironment = Object.fromEntries(
    Object.entries(process.env).filter(([key]) =>
      key.startsWith("LEMONSQUEEZY_VARIANT_ID_"),
    ),
  ...
  if (storeResult.error)
    throw new Error(
      storeResult.error.message || "Failed to read the Lemon Squeezy store",
    );
  ...
  for (const offer of COMMERCIAL_OFFERS) {
    ...
    if (productResult.error)
      throw new Error(`${offer.id}: failed to read the configured product`);
    if (pricesResult.error)
      throw new Error(`${offer.id}: failed to read the configured price`);
