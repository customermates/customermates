...
window.renderScene=(sceneId,progress,frame,frameCount)=>{
  ...
  for(const action of scene.actions){
    ...
    if(action.type==='swapState'){const target=cmRoot().querySelector('[data-state-target="'+action.target+'"]');if(!target)continue;const initial=target.querySelector('[data-state-initial]');const updated=target.querySelector('[data-state-updated]');const outgoing=1-clamp(value/.45);const incoming=clamp((value-.55)/.45);initial?.style.setProperty('opacity',String(outgoing));initial?.style.setProperty('transform','translateY('+(-4*(1-outgoing))+'px)');updated?.style.setProperty('opacity',String(incoming));updated?.style.setProperty('transform','translateY('+(4*(1-incoming))+'px)');}
