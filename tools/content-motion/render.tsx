...
const nodeAttributes = (
  ...
  "data-cm-min-content-fill": node.qa?.minContentFill,
...
window.renderScene=(sceneId,progress,frame,frameCount)=>{
  ...
  for(const action of scene.actions){
    ...
    if(action.type==='selectRadio'){const root=cmRoot().querySelector('[data-radio-target="'+action.target+'"]');if(root){const selected=value>=.5?action.value:root.dataset.radioInitial;root.dataset.value=selected;for(const item of root.querySelectorAll('[data-radio-value]')){const checked=item.dataset.radioValue===selected;item.dataset.state=checked?'checked':'unchecked';item.setAttribute('aria-checked',String(checked));const indicator=item.querySelector('[data-slot="radio-group-indicator"]');if(indicator){indicator.dataset.state=checked?'checked':'unchecked';indicator.style.display=checked?'flex':'none';}}}}
...
window.cmAuditLayout=()=>{
  ...
  for(const element of boxes){if(!visible(element))continue;const item=record(element);elements.push(item);
    ...
    if(element.dataset.cmInsetParent){const parent=cmNode(element.dataset.cmInsetParent);if(!parent)findings.push({code:'unknown-inset-parent',id:item.id});else{const a=element.getBoundingClientRect(),b=parent.getBoundingClientRect(),expected=cmSpace[element.dataset.cmInsetToken]??0,axis=element.dataset.cmInsetAxis??'x';if((axis==='x'||axis==='both')&&(Math.abs((a.left-b.left)-expected)>2||Math.abs((b.right-a.right)-expected)>2))findings.push({code:'inset-drift-x',id:item.id,parent:element.dataset.cmInsetParent,expected});if((axis==='y'||axis==='both')&&(Math.abs((a.top-b.top)-expected)>2||Math.abs((b.bottom-a.bottom)-expected)>2))findings.push({code:'inset-drift-y',id:item.id,parent:element.dataset.cmInsetParent,expected});}}
     ...
     if(element.dataset.cmMinContentFill){const minimum=Number(element.dataset.cmMinContentFill);const container=element.getBoundingClientRect();const leaves=[...element.querySelectorAll('[data-cm-id]')].filter((child)=>visible(child)&&!child.querySelector('[data-cm-id]'));const spans=leaves.map((child)=>child.getBoundingClientRect()).map((rect)=>({top:Math.max(container.top,rect.top),bottom:Math.min(container.bottom,rect.bottom)})).filter((span)=>span.bottom>span.top);const fill=spans.length?(Math.max(...spans.map((span)=>span.bottom))-Math.min(...spans.map((span)=>span.top)))/container.height:0;if(fill+.001<minimum)findings.push({code:'content-underfill',id:item.id,fill:Math.round(fill*1000)/1000,min:minimum});}
   }
