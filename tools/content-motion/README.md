...
An outer product surface may declare bounded `layout.adaptiveHeight` with `min`, `max`, `inset`, and a `top` or `center` anchor. The renderer derives its current height from visible leaf components, interpolates the boundary as their opacity changes, ignores focus and connector geometry, and never exceeds the declared base height. This keeps cards, forms, lists, and tables content-fit without story-specific height keyframes. Explicit height motion and adaptive height are mutually exclusive on the same node.
...
Focus nodes, connectors, and attached elements derive their geometry from stable target IDs and named anchors. They cannot supply independent `x` or `y` coordinates. Positioning is recalculated after frame transforms so feedback remains attached during camera motion. `qa.cameraTarget` marks the rendered surface whose live bounds the director must use for safe-area and occupancy checks; it is independent from `qa.critical`, which enforces the phone-size type floor on visible descendant copy. A content-driven surface may additionally declare `qa.minContentFill`; the audit measures the vertical span of its visible leaf components inside the rendered box and rejects a half-empty container even when its outer occupancy passes. The runtime audit reports out-of-bounds content, box and text-paint clipping, unintended declared-box overlap, asymmetric declared padding, declared-inset drift, alignment drift, focus misses, critical phone-size type below its configured threshold, and declared content underfill. Scene density budgets reject excess node count, text leaves, characters, or primary regions before rendering.
