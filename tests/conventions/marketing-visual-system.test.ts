...
type GoldenStoryVisualAcceptsTime = "t" extends keyof GoldenStoryVisualProps
  ? true
  : false;
...
function productProofRendererViolations(source: string) {
  return source.includes("product-proof") &&
    source.includes("GoldenStoryVisual")
...
const STATIC_VISUAL_SOURCE_RULES = [
  ...
  [
    "runtime animation",
    /(?:\b(?:requestAnimationFrame|setInterval|setTimeout)\s*\(|\.\s*animate\s*\()/u,
  ],
  ["time source", /\b(?:Date\.now|performance\.now)\s*\(/u],
  [
    "motion library",
    /(?:from\s+|import\s*)["'](?:framer-motion|motion\/react)["']/u,
  ],
...
function staticVisualSourceViolations(source: string) {
  return STATIC_VISUAL_SOURCE_RULES.flatMap(([label, pattern]) =>
...
function staticVisualStylesheetViolations(source: string) {
  const forbidden = [
    ...
    [
      "visual keyframes",
      /@keyframes\s+(?:connector|focal|golden|illustration|story|visual)[\w-]*/iu,
    ],
  ] as const;
  return forbidden.flatMap(([label, pattern]) =>
    pattern.test(source) ? [label] : [],
...
describe("marketing visual brief", () => {
  ...
  it("renders one benchmark in every placement and theme on its review sheet", () => {
    ...
    expect(
      markup.match(/data-golden-placement="(?:wide|split|narrow)"/gu),
    ).toHaveLength(6);
    expect(markup.match(/data-story-theme="(?:light|dark)"/gu)).toHaveLength(6);
    expect(markup).not.toMatch(
      /data-(?:candidate|composition|preferred|selected-variant|study)/u,
    );
    ...
    expect(germanMarkup.match(/data-story-pathway="converge"/gu)).toHaveLength(
      6,
    );
  ...
  it("allows a bespoke brief to use a pathway without copying its golden subject recipe", () => {
    ...
    const repeatedProviderSubjects =
      repeatedProvider.supportingSubjects as Array<Record<string, unknown>>;
  ...
  it("accepts approved native fixtures independently of source names and retains seeded pairings", () => {
    ...
    const sourceIndependentSubjects =
      sourceIndependent.supportingSubjects as Array<Record<string, unknown>>;
  ...
  it("accepts bounded provider-set, conversation, and Kanban composites without weakening subject reduction", () => {
    expect(VISUAL_PROVIDER_SET_FIXTURES["unified-inbox"].providers).toEqual([
      "gmail",
      "outlook",
      "linkedin",
      "whatsapp",
    ...
    expect(VISUAL_DEAL_BOARD_FIXTURES["demo-status-board"].statuses).toEqual([
      "deal-open",
      "deal-won",
      "deal-lost",
      "deal-abandoned",
    ...
    providerSet.supportingSubjects = [
      {
    ...
    expect(validateVisualBrief(providerSet)).toMatchObject({
      supportingSubjects: [{ fixtures: { providerSet: "unified-inbox" } }],
    });

    const contaminated = structuredClone(providerSet);
    const contaminatedSubjects = contaminated.supportingSubjects as Array<
      Record<string, unknown>
    >;
    contaminatedSubjects[0].fixtures = {
      provider: "gmail",
      providerSet: "unified-inbox",
    };
    expect(invalid(contaminated)).toThrow(/unrelated fixtures/u);

    const wrongConversation = structuredClone(providerSet);
    const wrongConversationFocal = wrongConversation.focalSubject as Record<
      string,
      unknown
    >;
    ...
    const board = clone(getGoldenVisualBrief("handoff", "en"));
    board.accentTarget = "deal-board";
    board.supportingSubjects = [
      {
    ...
    expect(validateVisualBrief(board)).toMatchObject({
      supportingSubjects: [{ fixtures: { dealBoard: "demo-status-board" } }],
    });
  });
  ...
  it("accepts seeded records that the source does not name", () => {
    ...
    const wrongStatusFocal = wrongStatus.focalSubject as Record<
      string,
      unknown
    >;
  ...
  it("requires every agent cue to select one approved native provider without inference", () => {
    ...
    const missingSubjects = missing.supportingSubjects as Array<
      Record<string, unknown>
    >;
    delete missingSubjects[0].agentProvider;

    const unsupported = clone(base);
    const unsupportedSubjects = unsupported.supportingSubjects as Array<
      Record<string, unknown>
    >;
    unsupportedSubjects[0].agentProvider = "codex";

    const misplaced = clone(base);
    const misplacedSubjects = misplaced.supportingSubjects as Array<
      Record<string, unknown>
    >;
    ...
    for (const [agentProvider, fixture] of Object.entries(
      VISUAL_AGENT_PROVIDER_FIXTURES,
    )) {
      const providerBrief = clone(base);
      const providerSubjects = providerBrief.supportingSubjects as Array<
        Record<string, unknown>
      >;
      ...
      if (validated.kind !== "brand-illustration")
      ...
      const markup = renderToStaticMarkup(
        createElement(NativeAgentProviderIdentity, {
          provider:
            agentProvider as keyof typeof VISUAL_AGENT_PROVIDER_FIXTURES,
        }),
      );
      ...
      if (agentProvider === "gemini")
        expect(markup).toContain("/images/brand/gemini-sparkle.svg");
    }
  });
  ...
  it("requires role-compatible people for authored human actions, drafts and records", () => {
    ...
    const wrongRecordFocal = wrongRecord.focalSubject as Record<
      string,
      unknown
    >;
...
describe("GoldenStoryVisual benchmark renderer", () => {
  ...
  it("recomposes fixture detail from essential to context to full density", () => {
    const converge = Object.fromEntries(
      VISUAL_PLACEMENTS.map((placement) => [
        placement,
        render(getGoldenVisualBrief("converge", "en"), placement),
      ]),
    );
    const handoff = Object.fromEntries(
      VISUAL_PLACEMENTS.map((placement) => [
        placement,
        render(getGoldenVisualBrief("handoff", "en"), placement),
      ]),
    );
    const focus = Object.fromEntries(
      VISUAL_PLACEMENTS.map((placement) => [
        placement,
        render(getGoldenVisualBrief("focus", "en"), placement),
      ]),
    );
    ...
    for (const placement of VISUAL_PLACEMENTS) {
      expect(handoff[placement]).toContain(
        'data-native-agent-provider="chatgpt"',
      );
  ...
  it("rejects generic content disguised with a golden ID and source checksum", () => {
    ...
    expect(() => render(brief)).toThrow(
      /exactly match the registered golden benchmark/u,
    );
  });
...
describe("golden benchmark connector geometry", () => {
  ...
  it("renders every connector as the exact full path to its authored target", () => {
    const paths = connectorPathTags(
      render(getGoldenVisualBrief("converge", "en"), "wide"),
    );
...
describe("marketing visual boundaries", () => {
  it("keeps product screens and UI chrome out while reusing only the approved native AI identity primitive", () => {
    ...
    expect(
      relativeFiles.filter((file, index) =>
        sources[index]?.includes("@/components/ai-connection/ai-client-logo"),
      ),
  ...
  it("plants and enforces the static-only visual source boundary", () => {
    ...
    expect(
      staticVisualSourceViolations(
        'import { motion } from "motion/react"; node.animate([], {}); useSceneClock();',
      ),
    ...
    expect(
      files.filter((file) => /(?:film|motion|storyboard|video)/iu.test(file)),
    ).toEqual([]);
  ...
  it("keeps the authored style guide static without banning generic reduced-motion UI behavior", () => {
    ...
    expect(
      relativePaths.filter((file) => /\/(?:motion|frame)\//u.test(file)),
    ).toEqual([]);
    expect(
      relativePaths.filter((file) =>
        /(?:motion-study|motion-storyboard)/u.test(file),
      ),
    ).toEqual([]);
    expect(
      staticStyleguideSourceViolations('<video src="benchmark.webm" />'),
    ).toEqual(["embedded moving media", "moving media asset"]);
    expect(
      staticStyleguideSourceViolations(
        '<picture><source srcSet="benchmark.avif" /></picture>',
      ),
    ).toEqual([]);
  ...
  it("removes the retired illustration and film CSS", () => {
    ...
    expect(stylesheet).not.toMatch(
      /illustration-(?:body|detail|edge|accent|mark)-(?:in|draw|pop)/u,
    );
    expect(stylesheet).not.toContain("data-illustration-play");
    expect(stylesheet).not.toContain(".scene-frame-film");
  });

  it("detects planted proof rendering and keeps proof outside GoldenStoryVisual", () => {
    expect(
      productProofRendererViolations(
        'const kind = "product-proof"; return <GoldenStoryVisual />;',
      ),
    ).toEqual(["product-proof uses GoldenStoryVisual"]);

    const directory = join(REPO_ROOT, "components", "marketing", "visuals");
    const violations = walkFiles(directory, (file) =>
      /\.[cm]?[jt]sx?$/u.test(file),
    )
      .filter((file) => file.includes("product-proof"))
      .flatMap((file) =>
        productProofRendererViolations(readFileSync(file, "utf8")),
      );
  ...
  it("keeps layout selection out of the generic and golden public APIs", () => {
    ...
    const goldens = readFileSync(
      join(REPO_ROOT, "components", "marketing", "visuals", "goldens.ts"),
      "utf8",
    );
    ...
    expect(contract).toContain(
      'VISUAL_PATHWAYS = ["converge", "handoff", "focus"]',
    );
    expect(contract).not.toMatch(
      /DEFAULT_VISUAL_VARIANT|VISUAL_VARIANTS|VisualVariant|selectedVariant\s*:/u,
    );
    ...
    expect(renderer).not.toMatch(
      /\b(?:candidate|composition|preferred|study|variant)\s*:/u,
    );
    ...
    expect(publicApi).not.toMatch(
      /Candidate|Composition|Preferred|Study|Variant|GOLDEN_(?:COMPOSITION|PREFERRED)/u,
    );
  });
  ...
  it("keeps native fixture values closed and source-backed", () => {
    expect(Object.keys(VISUAL_AGENT_PROVIDER_FIXTURES)).toEqual([
      "chatgpt",
      "claude",
      "cursor",
      "gemini",
    ...
    expect(
      NATIVE_VISUAL_FIXTURE_SOURCES.filter(
