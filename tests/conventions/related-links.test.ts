...
function entries(collection: string, locale: string): Entry[] {
  return walkFiles(join(REPO_ROOT, "content", collection, locale), (path) => path.endsWith(".mdx")).map((path) => {
    ...
    const frontmatter = /^---\n(.*?)\n---\n?/su.exec(source);
    const data = frontmatter ? (parse(frontmatter[1]) as { acquisition?: { relatedHrefs?: string[] } }) : undefined;
    const authoredHrefs = data?.acquisition?.relatedHrefs;

    return {
      hrefs:
        authoredHrefs ??
        (block ? [...block[1].matchAll(/<RelatedPage\s+href="([^"]+)"\s*\/>/gu)].map((match) => match[1]) : []),
...
describe("related links", () => {
  it("resolves every supported route family and rejects malformed or missing targets", () => {
    const alternativeTitle = (competitor: string) => `Customermates alternative to ${competitor}`;
    const calls: string[] = [];
    const resolver =
    ...
    for (const segment of RELATED_ROUTE_SEGMENTS) {
      const target = resolveRelatedTarget(`/${segment}/example`, "en", alternativeTitle, resolvers);
      expect(target.title).toBe(`${segment}:example`);
      expect(target.description.length).toBeGreaterThan(0);
    }
    expect(calls).toEqual(RELATED_ROUTE_SEGMENTS.map((segment) => `${segment}:example:en`));

    for (const href of ["/docs/mcp/anything", "/docs", "/unknown/mcp", "docs/mcp"])
      expect(() => resolveRelatedTarget(href, "en", alternativeTitle, resolvers)).toThrow(
        "is not a related-page route",
      );

    expect(() => resolveRelatedTarget("/docs/missing", "en", alternativeTitle, resolvers)).toThrow(
      "resolves to no published page",
    );
  });

  it("keeps every route family wired to its published content source", () => {
    const component = readFileSync(join(REPO_ROOT, "components/marketing/related-pages.tsx"), "utf8");
    for (const segment of RELATED_ROUTE_SEGMENTS)
      expect(component, `${segment} resolver is missing`).toMatch(new RegExp(`^  ${segment}:`, "mu"));
  });
      ...
      for (const locale of CONTENT_LOCALES) {
        ...
        for (const entry of all) {
          ...
          for (const href of entry.hrefs) {
            if (!published.has(href)) problems.push(`${where}: ${href} names no published page`);
            else if (href === `${base}/${entry.slug}`) problems.push(`${where}: links to itself`);
          }
        }
  ...
  it("validates acquisition blog links against published non-self targets", () => {
    for (const locale of CONTENT_LOCALES) {
      ...
      for (const entry of entries("blog-posts", locale).filter(({ isAcquisition }) => isAcquisition)) {
        ...
        for (const href of entry.hrefs) {
          if (!published.has(href)) problems.push(`${where}: ${href} names no published page`);
          else if (href === `/blog/${entry.slug}`) problems.push(`${where}: links to itself`);
        }
      }
  ...
  it("gives every blog article the same four-link and localized CTA shell", () => {
    ...
    const route = readFileSync(join(REPO_ROOT, "app/[locale]/(static)/blog/[slug]/page.tsx"), "utf8");
    expect(route).toContain("selectRelatedSlugs(slug, ringSlugs)");
    expect(route).toContain("page.data.acquisition?.relatedHrefs ?? relatedHrefs");
    ...
    for (const locale of CONTENT_LOCALES) {
      const input = readFileSync(join(REPO_ROOT, "content", "blog", locale, "blog.mdx"), "utf8");
      const frontmatter = /^---\n(.*?)\n---\n?/su.exec(input);
      const cta = frontmatter ? (parse(frontmatter[1]) as { cta?: Record<string, string> }).cta : undefined;
    ...
    for (const locale of CONTENT_LOCALES) {
      const inbound = new Map<string, number>(
        Object.entries(COLLECTIONS).flatMap(([collection, base]) =>
          entries(collection, locale).map(({ slug }) => [`${base}/${slug}`, 0] as const),
        ),
      );

      for (const collection of Object.keys(COLLECTIONS))
        for (const entry of entries(collection, locale))
          for (const href of entry.hrefs) if (inbound.has(href)) inbound.set(href, (inbound.get(href) ?? 0) + 1);
