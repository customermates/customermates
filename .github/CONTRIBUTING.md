...
Nested dropdown submenus are the one shape the contract cannot fully rescue. A submenu opens beside its parent, so once the parent consumes the width of a small screen there is nowhere for it to go: Radix flips it correctly and still runs out of room, and the reported available width goes negative. A viewport cap in `styles/globals.css` stops it exceeding the screen, but prefer a flat menu, or the responsive Dialog/Drawer path, over a submenu on any surface that has to work on a phone. There are currently no `DropdownMenuSubContent` call sites in the app.
